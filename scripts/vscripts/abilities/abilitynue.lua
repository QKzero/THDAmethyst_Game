function OnNueExSpellStart(keys)
    ...
    local effectIndex = ParticleManager:CreateParticle(
        "particles/units/heroes/hero_templar_assassin/templar_assassin_trap_explode.vpcf", PATTACH_CUSTOMORIGIN, caster)
...
function OnNue01SpellStart(keys)
    ...
    caster:SetModifierStackCount("modifier_nue_01_extradamage", caster,
        caster:GetModifierStackCount("modifier_nue_01_extradamage", caster) + 1)

    local effectIndex = ParticleManager:CreateParticle("particles/heroes/nue/ability_nue_01_ball.vpcf",
...
function OnNue01AttackLanded(keys)
    ...
    local effectIndex = ParticleManager:CreateParticle(
        "particles/units/heroes/hero_templar_assassin/templar_assassin_meld_hit.vpcf", PATTACH_CUSTOMORIGIN, caster)
...
function OnNue04PhaseStart(keys)
    local caster = EntIndexToHScript(keys.caster_entindex)
    keys.ability.ufoIndex = ParticleManager:CreateParticle("particles/heroes/nue/ability_nue_04_light_ufo.vpcf",
        PATTACH_CUSTOMORIGIN, nil)
    ParticleManager:SetParticleControl(keys.ability.ufoIndex, 0, caster:GetOrigin())
    ParticleManager:DestroyParticleSystemTime(keys.ability.ufoIndex, 3.0)
end
function OnNue04PhaseInterrupted(keys)
    if (keys.ability.ufoIndex ~= null) then
        ParticleManager:DestroyParticleSystem(keys.ability.ufoIndex, true)
    end
end
...
function OnNue04Start(keys)
    ...
    local ufoMoveIndex = ParticleManager:CreateParticle("particles/heroes/nue/ability_nue_04_warning_circle.vpcf",
        PATTACH_CUSTOMORIGIN, nil)
    ParticleManager:SetParticleControl(ufoMoveIndex, 0, targetPoint)
    print(keys.Radius)
    ParticleManager:SetParticleControl(ufoMoveIndex, 1, Vector(keys.Radius, 0, 0))
    ParticleManager:SetParticleControl(ufoMoveIndex, 3, Vector(153, 50, 204))
    AddFOWViewer(DOTA_TEAM_GOODGUYS, targetPoint, 700, 1.5, false)
    AddFOWViewer(DOTA_TEAM_BADGUYS, targetPoint, 700, 1.5, false)

    caster:AddNoDraw()
    caster:SetContextThink(DoUniqueString("OnNue04SpellThink"), function()
        if GameRules:IsGamePaused() then
            return 0.03
        end
        caster:RemoveNoDraw()
        local targets = FindUnitsInRadius(caster:GetTeam(), targetPoint, nil, keys.Radius, DOTA_UNIT_TARGET_TEAM_ENEMY,
            keys.ability:GetAbilityTargetType(), 0, FIND_CLOSEST, false)

        local effectIndex = ParticleManager:CreateParticle("particles/heroes/nue/ability_nue_04.vpcf",
            PATTACH_CUSTOMORIGIN, caster)
        ...
        for k, v in pairs(targets) do
            ...
            UtilStun:UnitStunTarget(caster, v,
                keys.stun_duration + FindTelentValue(caster, "special_bonus_unique_nue_2"))
            UnitDamageTarget(damage_table)
        end
        FindClearSpaceForUnit(caster, targetPoint, true)
        caster:StartGesture(ACT_DOTA_CAST_ABILITY_4_END)
        local ufoIndex2 = ParticleManager:CreateParticle("particles/heroes/nue/ability_nue_04_light_ufo.vpcf",
            PATTACH_CUSTOMORIGIN, caster)
        ParticleManager:SetParticleControl(ufoIndex2, 0, caster:GetOrigin())
        ParticleManager:DestroyParticleSystemTime(ufoIndex2, 3.0)
