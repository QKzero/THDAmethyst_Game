...
function Ran01_OnSpellStart(keys)
    ...
    if flag_unit then
        ...
        local caster_pfx = ParticleManager:CreateParticle(
            "particles/units/heroes/hero_vengeful/vengeful_nether_swap.vpcf", PATTACH_ABSORIGIN, caster)
        ParticleManager:SetParticleControlEnt(caster_pfx, 0, caster, PATTACH_POINT_FOLLOW, "attach_hitloc",
            caster:GetAbsOrigin(), true)
        ParticleManager:SetParticleControlEnt(caster_pfx, 1, flag_unit, PATTACH_POINT_FOLLOW, "attach_hitloc",
            flag_unit:GetAbsOrigin(), true)

        -- Play target particle
        local target_pfx = ParticleManager:CreateParticle(
            "particles/units/heroes/hero_vengeful/vengeful_nether_swap_target.vpcf", PATTACH_ABSORIGIN, flag_unit)
        ParticleManager:SetParticleControlEnt(target_pfx, 0, flag_unit, PATTACH_POINT_FOLLOW, "attach_hitloc",
            flag_unit:GetAbsOrigin(), true)
        ParticleManager:SetParticleControlEnt(target_pfx, 1, caster, PATTACH_POINT_FOLLOW, "attach_hitloc",
            caster:GetAbsOrigin(), true)
    else
        caster:EmitSound("Hero_Techies.Sign")
        -- create flag unit
        flag_unit = CreateUnitByName(keys.FlagUnitName, target, true, caster, caster, caster:GetTeam())
...
function Ran03_OnSpellStart(keys)
    ...
    local effectIndex = ParticleManager:CreateParticle("particles/heroes/ran/ability_ran_03_laser.vpcf",
    ...
    if jumpAmount > 1 then
        caster:SetContextThink("ran03_lazer_jumping", function()
            ...
            local enemies = FindUnitsInRadius(caster:GetTeamNumber(), targetLoc, nil, keys.JumpRadius,
            ...
            if next_target then
                effectIndex = ParticleManager:CreateParticle("particles/heroes/ran/ability_ran_03_laser.vpcf",
                    PATTACH_CUSTOMORIGIN, caster)
                ...
                if target_unit and not target_unit:IsNull() then
                    ...
                    ParticleManager:SetParticleControlEnt(effectIndex, 0, target_unit, 5, "attach_hitloc", targetLoc,
                        true)
                    ParticleManager:SetParticleControlEnt(effectIndex, 9, target_unit, 5, "attach_hitloc", targetLoc,
                        true)
                else
                ...
                ParticleManager:SetParticleControlEnt(effectIndex, 1, next_target, 5, "attach_hitloc",
                    next_target:GetAbsOrigin(), true)
            ...
            if jump_count >= jumpAmount or target_unit == nil then
                return nil
            end
