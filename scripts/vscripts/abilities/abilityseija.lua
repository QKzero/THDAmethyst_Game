...
function ability_thdots_seija01:OnProjectileHitHandle(hTarget, vLocation, iProjectileHandle)
	...
	local ability = self
	local damage = self.damage + self.int_bonus * caster:GetIntellect(false)
...
function ability_thdots_seija02:OnProjectileHitHandle(hTarget, vLocation, iProjectileHandle)
	...
	--target:EmitSound("Voice_Thdots_Seija.AbilitySeija02_2") here have warning
	target:AddNewModifier(caster,ability, "modifier_ability_thdots_seija02_debuff",{duration = self.duration_slow * (1 - target:GetStatusResistance())})
	local damage_dealt = UnitDamageTarget(damageTable)
	for i = 1,#self.projectile_table do
		if self.projectile_table[i].barrage == iProjectileHandle then
...
function modifier_ability_thdots_seija02_debuff:GetModifierMoveSpeedBonus_Percentage()
	return self:GetAbility():GetSpecialValueFor("movement_slow") end
...
function ability_thdots_seija03:OnSpellStart()
	...
	if target:IsHero() then
		self.intellect_all 			= target:GetIntellect(false)
...
function modifier_ability_thdots_seija03:OnCreated()
	...
	self.intellect_all 		= self.parent:GetIntellect(false)
