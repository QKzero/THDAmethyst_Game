...
function OnKaguya01SpellThink(keys)
    ...
    local damageVector = Vector(math.cos(rollRad) * forwardCos - math.sin(rollRad) * forwardSin,
        forwardSin * math.cos(rollRad) + forwardCos * math.sin(rollRad), 0) * 350 + vecCaster
    local effectIndex
    if (count % 3 == 0) then
        effectIndex = ParticleManager:CreateParticle("particles/thd2/heroes/kaguya/ability_kaguya01_light.vpcf",
            PATTACH_CUSTOMORIGIN, nil)
    elseif (count % 3 == 1) then
        effectIndex = ParticleManager:CreateParticle("particles/thd2/heroes/kaguya/ability_kaguya01_light_green.vpcf",
            PATTACH_CUSTOMORIGIN, nil)
    elseif (count % 3 == 2) then
        effectIndex = ParticleManager:CreateParticle("particles/thd2/heroes/kaguya/ability_kaguya01_light_red.vpcf",
            PATTACH_CUSTOMORIGIN, nil)
    end
    count = count + 1
    ...
    for _, v in pairs(targets) do
        ...
        local deal_damage = keys.ability:GetAbilityDamage() + FindTelentValue(caster, "special_bonus_unique_kaguya_2") +
                                v.kaguya01_exdamage_count * keys.ExDamage
        ...
        if v and not v:IsNull() and caster:HasModifier("modifier_item_wanbaochui") then
            if v.kaguya01_exdamage_count == 1 then
                keys.ability:ApplyDataDrivenModifier(caster, v, "modifier_thdots_kaguya_wanbaochui_debuff_1", {
                    duration = 3
                })
            elseif v.kaguya01_exdamage_count == 2 then
                keys.ability:ApplyDataDrivenModifier(caster, v, "modifier_thdots_kaguya_wanbaochui_debuff_2", {
                    duration = 3
                })
            elseif v.kaguya01_exdamage_count == 3 then
                keys.ability:ApplyDataDrivenModifier(caster, v, "modifier_thdots_kaguya_wanbaochui_debuff_3", {
                    duration = 3
                })
            elseif v.kaguya01_exdamage_count == 4 then
                keys.ability:ApplyDataDrivenModifier(caster, v, "modifier_thdots_kaguya_wanbaochui_debuff_4", {
                    duration = 3
                })
            elseif v.kaguya01_exdamage_count == 5 then
                keys.ability:ApplyDataDrivenModifier(caster, v, "modifier_thdots_kaguya_wanbaochui_debuff_5", {
                    duration = 3
                })
            elseif v.kaguya01_exdamage_count == 6 then
                keys.ability:ApplyDataDrivenModifier(caster, v, "modifier_thdots_kaguya_wanbaochui_debuff_6", {
                    duration = 3
                })
                caster:GiveMana(caster:GetMaxMana() * 0.3)
            elseif v.kaguya01_exdamage_count == 7 then
                keys.ability:ApplyDataDrivenModifier(caster, v, "modifier_thdots_kaguya_wanbaochui_debuff_7", {
                    duration = 3
                })
...
function OnKaguya03SpellStart(keys)
    local caster = EntIndexToHScript(keys.caster_entindex)
    local dummy = CreateUnitByName("npc_dummy_unit", caster:GetOrigin(), false, caster, caster, caster:GetTeam())
    local ability_dummy_unit = dummy:FindAbilityByName("ability_dummy_unit")
    ability_dummy_unit:SetLevel(1)

    dummy:AddAbility("thd_darkness")
    local darkness = dummy:FindAbilityByName("thd_darkness")
    darkness:SetLevel(3)
...
function OnKaguya03ManaRegen(keys)
    ...
    if (GameRules:IsDaytime() == true) then
        ...
        if FindTelentValue(caster, "special_bonus_unique_kaguya_5") == 1 then
            local bonusMana = 1.5 * (keys.ManaRegen + keys.BonusRegen * GameRules:GetGameTime() / keys.increaseTime) /
                                  10
            caster:SetMana(caster:GetMana() + bonusMana)
        else
...
function OnKaguya04SpellStart(keys)
    local caster = EntIndexToHScript(keys.caster_entindex)
    caster.ability_kaguya_04_point = keys.ability:GetCursorPosition()
    caster.effectIndex = ParticleManager:CreateParticle("particles/heroes/kaguya/ability_kaguya04_effect.vpcf",
        PATTACH_CUSTOMORIGIN, caster)
    ParticleManager:SetParticleControl(caster.effectIndex, 0, keys.ability:GetCursorPosition())
    ParticleManager:SetParticleControl(caster.effectIndex, 1, keys.ability:GetCursorPosition())
    ParticleManager:SetParticleControl(caster.effectIndex, 3, keys.ability:GetCursorPosition())
    ParticleManager:SetParticleControl(caster.effectIndex, 6, Vector(1, 0, 0));
    ParticleManager:DestroyParticleSystem(caster.effectIndex, false)
    caster:EmitSound("Hero_Phoenix.SuperNova.Cast")

    local dummy = CreateUnitByName("npc_dummy_unit", caster.ability_kaguya_04_point, false, caster, caster,
        caster:GetTeam())
    local ability_dummy_unit = dummy:FindAbilityByName("ability_dummy_unit")
    ability_dummy_unit:SetLevel(1)
    ...
    caster.kaguya04dummy:SetContextThink("killself", function()
        if caster.kaguya04dummy ~= nil and not caster.kaguya04dummy:IsNull() then
    ...
    caster:SetContextThink(DoUniqueString("kaguya_04_SpellStart"), function()
        if GameRules:IsGamePaused() then
            return 0.04
        end

        if (caster.ability_kaguya_04_radius <= 300 + caster.ability_kaguya_04_mana_store) and
            (caster.ability_kaguya_04_radius <= 1000) and caster:IsAlive() then
            caster.ability_kaguya_04_radius = caster.ability_kaguya_04_radius + keys.IncreaseRadius
            ParticleManager:SetParticleControl(caster.effectIndex, 6,
                Vector(1 + (caster.ability_kaguya_04_radius - 100) / 350, 0, 0));
        else
            caster.ability_kaguya_04_radius = 100
            caster.ability_kaguya_04_mana_store = 0
            if (caster.effectIndex ~= nil) then
                local effectIndex = ParticleManager:CreateParticle(
                    "particles/heroes/kaguya/ability_kaguya04_explosion.vpcf", PATTACH_CUSTOMORIGIN, caster)
        ...
        if caster.kaguya04aimrange ~= nil then
            ...
            ParticleManager:SetParticleControl(caster.kaguya04aimrange2, 0,
                Vector((caster.ability_kaguya_04_radius * 0.9), 0, 0))
            ParticleManager:SetParticleControl(caster.kaguya04aimrange2, 1,
                Vector((caster.ability_kaguya_04_radius * 0.9), 0, 0))
            ParticleManager:SetParticleControl(caster.kaguya04aimrange3, 0,
                Vector((caster.ability_kaguya_04_radius * 0.8), 0, 0))
            ParticleManager:SetParticleControl(caster.kaguya04aimrange3, 1,
                Vector((caster.ability_kaguya_04_radius * 0.8), 0, 0))
            ParticleManager:SetParticleControl(caster.kaguya04aimrange4, 0,
                Vector((caster.ability_kaguya_04_radius * 0.7), 0, 0))
            ParticleManager:SetParticleControl(caster.kaguya04aimrange4, 1,
                Vector((caster.ability_kaguya_04_radius * 0.7), 0, 0))
            ParticleManager:SetParticleControl(caster.kaguya04aimrange5, 0,
                Vector((caster.ability_kaguya_04_radius * 0.6), 0, 0))
            ParticleManager:SetParticleControl(caster.kaguya04aimrange5, 1,
                Vector((caster.ability_kaguya_04_radius * 0.6), 0, 0))
            ParticleManager:SetParticleControl(caster.kaguya04aimrange6, 0,
                Vector((caster.ability_kaguya_04_radius * 0.5), 0, 0))
            ParticleManager:SetParticleControl(caster.kaguya04aimrange6, 1,
                Vector((caster.ability_kaguya_04_radius * 0.5), 0, 0))
        end
    ...
    if (lostHp > 0) then
        if FindTelentValue(caster, "special_bonus_unique_kaguya_6") == 1 then
            ...
            if (lostHp <= keys.HpRegen) then
                ...
                if (caster:GetMana() >= keys.HpRegen * keys.CostMana) then
                    ...
                    caster.ability_kaguya_04_mana_store = caster.ability_kaguya_04_mana_store + keys.HpRegen *
                                                              keys.CostMana
                end
            end
        end
        ...
        if (caster.ability_kaguya_04_mana_store ~= nil) then
            if (caster.ability_kaguya_04_mana_store >= 0) then
                caster.ability_kaguya_04_mana_store = caster.ability_kaguya_04_mana_store - caster:GetMaxMana() *
                                                          keys.Decrease_speed
                if (caster.ability_kaguya_04_mana_store < 0) then
                    caster.ability_kaguya_04_mana_store = 0
                end
