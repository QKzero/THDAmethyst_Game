...
function OnKaguya04SpellStart(keys)
	...
	caster:SetContextThink(DoUniqueString("kaguya_04_SpellStart"),
		function()
			if GameRules:IsGamePaused() then return 0.04 end

			if(caster.ability_kaguya_04_radius<=300+caster.ability_kaguya_04_mana_store) and (caster.ability_kaguya_04_radius<=1000) and caster:IsAlive() then
				caster.ability_kaguya_04_radius = caster.ability_kaguya_04_radius + keys.IncreaseRadius
				ParticleManager:SetParticleControl(caster.effectIndex, 6, Vector(1+(caster.ability_kaguya_04_radius-100)/350,0,0));
			else
				caster.ability_kaguya_04_radius = 100
				caster.ability_kaguya_04_mana_store =0
