...
LinkLuaModifier("modifier_ability_thdots_iku_knockback_rooted","scripts/vscripts/abilities/abilityiku.lua",LUA_MODIFIER_MOTION_NONE)
function modifier_ability_thdots_iku_knockback_rooted:IsHidden() 		return true end
function modifier_ability_thdots_iku_knockback_rooted:IsPurgable()		return false end				--被驱散后减层数会报空
function modifier_ability_thdots_iku_knockback_rooted:RemoveOnDeath() 	return true end
function modifier_ability_thdots_iku_knockback_rooted:IsDebuff()		return true end
...
function modifier_ability_thdots_iku_knockback_rooted:CheckState()
	if self:GetParent():HasModifier("modifier_item_dragon_star_buff") or self:GetParent():HasModifier("modifier_meirin02_buff") then	--同牵引
		return
	end
	return {
