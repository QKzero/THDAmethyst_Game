...
function Mystia01OnSpellStart(keys)
    ...
    for _,v in pairs(targets) do
        local DamageTable = {
                           ...
                           victim = v, 
                           attacker = caster, 
                           damage = keys.damage + agi * keys.agility_bonus , 
...
function Mystia04OnSpellStart(keys)
    ...
    local duration = keys.ability:GetSpecialValueFor("duration")
    local duration_darkness = keys.ability:GetSpecialValueFor("duration_darkness")
    ...
    if caster:HasScepter() then
      for spell_id = 0,caster:GetAbilityCount()-1 do
        local ability = caster:GetAbilityByIndex(spell_id)
        if ability ~= nil then
          if ability:GetAbilityType() == 0 then
      ...
      GameRules:BeginNightstalkerNight(duration_darkness)
...
function mystiaExOnSpellStart( keys )
  ...
  local flymodifier = caster:FindModifierByName("modifier_mystiaEx_fly")
  if caster:HasModifier("modifier_mystiaEx_fly") then
    flymodifier:SetDuration(4 + FindTelentValue(caster,"special_bonus_unique_mystia_4") + flymodifier:GetRemainingTime(), true)
  else
