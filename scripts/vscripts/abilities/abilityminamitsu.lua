...
function OnMinamitsu02SpellStart(keys)
	...
	--ParticleManager:SetParticleControl(effectIndex, 3, vecHook)
	local XmarkEffect = ParticleManager:CreateParticleForTeam("particles/units/heroes/hero_kunkka/kunkka_ghostship_marker_x.vpcf", PATTACH_WORLDORIGIN, nil, caster:GetTeam())
	ParticleManager:SetParticleControl(XmarkEffect, 0, targetPoint)
	local MarkSplashEffect = ParticleManager:CreateParticleForTeam("particles/units/heroes/hero_kunkka/kunkka_ghostship_marker_splash.vpcf", PATTACH_WORLDORIGIN, nil, caster:GetTeam())
	ParticleManager:SetParticleControl(MarkSplashEffect, 0, targetPoint)
	local MarkSplashRingEffect = ParticleManager:CreateParticleForTeam("particles/units/heroes/hero_kunkka/kunkka_ghostship_marker_splash_ring.vpcf", PATTACH_WORLDORIGIN, nil, caster:GetTeam())
	ParticleManager:SetParticleControl(MarkSplashRingEffect, 0, targetPoint)
	local MarkWarpEffect = ParticleManager:CreateParticleForTeam("particles/units/heroes/hero_kunkka/kunkka_ghostship_marker_warp.vpcf", PATTACH_WORLDORIGIN, nil, caster:GetTeam())
	...
	Timers:CreateTimer(0.03, function ()
		...
		if distance >= 0 then
			vecHook =  vecHook + Vector(math.cos(rad)*speed,math.sin(rad)*speed,0)
			ParticleManager:SetParticleControl(effectIndex, 3, Vector(vecHook.x, vecHook.y, GetGroundPosition(targetPoint, nil).z))
		else
			ParticleManager:DestroyParticleSystem(effectIndex,true)
...
function OnMinamitsu04Spawn(keys)
	...
	local caster = keys.caster
	--创建船
	local minamitsu_04_ship = CreateUnitByName("ability_minamitsu_04_ship", 
			caster:GetOrigin(),
			false,
			caster,
			caster,
		...
		if ability ~= nil then
			if hero.IsDriving then
				...
				if FindTelentValue(caster,"special_bonus_unique_minamitsu_5")==0 then
					ability:ApplyDataDrivenModifier( hero, hero, "modifier_minamitsu04_ship_destroy", {Duration = keys.DrownDuration} )
				end
			end
		end
	end
