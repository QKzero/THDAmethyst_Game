function OnMinamitsu01SpellStart(keys)
    ...
    local effectIndex = ParticleManager:CreateParticle("particles/heroes/minamitsu/ability_minamitsu_01.vpcf",
    ...
    caster:SetContextThink("ability_thdots_minamitsu_01_stage_01", function()
        ...
        if distance < keys.range then
            ...
            local targets = FindUnitsInRadius(caster:GetTeam(), vecHook, nil, keys.hitbox_radius,
                keys.ability:GetAbilityTargetTeam(), targettype, 0, FIND_CLOSEST, false)
            for i = #targets, 1, -1 do
                if targets[i] == caster or targets[i]:GetClassname() == "npc_dota_roshan" or targets[i]:GetUnitName() ==
                    "ability_minamitsu_04_ship" or targets[i]:HasModifier("dummy_unit") or
                    targets[i]:HasModifier("modifier_item_dragon_star_buff") or
                    targets[i]:HasModifier("modifier_meirin02_buff") then
...
function OnMinamitsu01Back(vecHook, target, rad, distance, effectIndex, keys)
    ...
    caster:SetContextThink("ability_thdots_minamitsu_01_stage_02", function()
        if GameRules:IsGamePaused() then
            return 0.03
        end
        distance = distance - speed
        if distance > 0 then
            ...
            if target == nil then
                local targets = FindUnitsInRadius(caster:GetTeam(), vecHook, nil, keys.hitbox_radius,
                    keys.ability:GetAbilityTargetTeam(), targettype, 0, FIND_CLOSEST, false)
                for i = #targets, 1, -1 do
                    if targets[i] == caster or targets[i]:GetClassname() == "npc_dota_roshan" or
                        targets[i]:GetUnitName() == "ability_minamitsu_04_ship" or targets[i]:HasModifier("dummy_unit") or
                        targets[i]:HasModifier("modifier_item_dragon_star_buff") or
                        targets[i]:HasModifier("modifier_meirin02_buff") then
                        table.remove(targets, i)
                    end
...
function OnMinamitsu02SpellStart(keys)
    ...
    local effectIndex = ParticleManager:CreateParticle("particles/heroes/minamitsu/ability_minamitsu_02_body.vpcf",
        PATTACH_WORLDORIGIN, nil)
    -- ParticleManager:SetParticleControl(effectIndex, 3, vecHook)
    local XmarkEffect = ParticleManager:CreateParticleForTeam(
        "particles/units/heroes/hero_kunkka/kunkka_ghostship_marker_x.vpcf", PATTACH_WORLDORIGIN, nil, caster:GetTeam())
    ParticleManager:SetParticleControl(XmarkEffect, 0, targetPoint)
    local MarkSplashEffect = ParticleManager:CreateParticleForTeam(
        "particles/units/heroes/hero_kunkka/kunkka_ghostship_marker_splash.vpcf", PATTACH_WORLDORIGIN, nil,
        caster:GetTeam())
    ParticleManager:SetParticleControl(MarkSplashEffect, 0, targetPoint)
    local MarkSplashRingEffect = ParticleManager:CreateParticleForTeam(
        "particles/units/heroes/hero_kunkka/kunkka_ghostship_marker_splash_ring.vpcf", PATTACH_WORLDORIGIN, nil,
        caster:GetTeam())
    ParticleManager:SetParticleControl(MarkSplashRingEffect, 0, targetPoint)
    local MarkWarpEffect = ParticleManager:CreateParticleForTeam(
        "particles/units/heroes/hero_kunkka/kunkka_ghostship_marker_warp.vpcf", PATTACH_WORLDORIGIN, nil,
        caster:GetTeam())
    ParticleManager:SetParticleControl(MarkWarpEffect, 0, targetPoint)
    ...
    Timers:CreateTimer(0.03, function()
        ...
        if distance >= 0 then
            ...
            ParticleManager:SetParticleControl(effectIndex, 3,
                Vector(vecHook.x, vecHook.y, GetGroundPosition(targetPoint, nil).z))
        else
            ParticleManager:DestroyParticleSystem(effectIndex, true)
            ParticleManager:DestroyParticleSystem(XmarkEffect, true)
            ParticleManager:DestroyParticleSystem(MarkSplashEffect, true)
            ParticleManager:DestroyParticleSystem(MarkSplashRingEffect, true)
            ParticleManager:DestroyParticleSystem(MarkWarpEffect, true)
            local effectIndex2 = ParticleManager:CreateParticle("particles/heroes/minamitsu/ability_minamitsu_02.vpcf",
                PATTACH_CUSTOMORIGIN, nil)
            ParticleManager:SetParticleControl(effectIndex2, 0, GetGroundPosition(vecHook, nil))
            ParticleManager:SetParticleControl(effectIndex2, 3, GetGroundPosition(vecHook, nil))
...
function OnMinamitsu02Vortex(keys)
    ...
    local rad = GetRadBetweenTwoVec2D(vecCaster, targetPoint)
    local timeCount = 0
    keys.ability_minamitsu_02_group = {}

    Timers:CreateTimer(0.03, function()
        timeCount = timeCount + 0.03
        local targets = FindUnitsInRadius(caster:GetTeam(), targetPoint, nil, keys.Radius,
            keys.ability:GetAbilityTargetTeam(), keys.ability:GetAbilityTargetType(), 0, FIND_CLOSEST, false)
        for k, v in pairs(targets) do
            if v:IsNull() == false and v ~= nil then
                ...
                UnitDamageTarget(damage_table)
                if v and not v:IsNull() and not v:HasModifier("modifier_item_dragon_star_buff") and
                    not v:HasModifier("modifier_meirin02_buff") then
                    table.insert(keys.ability_minamitsu_02_group, v)
                    if v:IsNull() == false and v ~= nil then
                        ...
                        if v:HasModifier("modifier_minamitsu02_vortex_target") == false then
                            ...
                            if distance > 30 then
                                v:SetOrigin(Vector(vecTarget.x - math.cos(targetRad - math.pi / 3) * keys.VortexSpeed *
                                                       1.5, vecTarget.y - math.sin(targetRad - math.pi / 3) *
                                    keys.VortexSpeed * 1.5, vecTarget.z))
                            else
                                v:AddNoDraw()
...
function OnMinamitsu02VortexEnd(keys)
    ...
    Timers:CreateTimer(0.03, function()
        timeCount = timeCount + 0.03
        speed = speed - g
        if keys.ability_minamitsu_02_group ~= nil then
            for k, v in pairs(keys.ability_minamitsu_02_group) do
                if v:IsNull() == false and v ~= nil then
            ...
            if timeCount >= 1.0 then
                for k2, v2 in pairs(keys.ability_minamitsu_02_group) do
...
function OnMinamitsu03SpellStart(keys)
    ...
    local effectIndex = ParticleManager:CreateParticle("particles/heroes/minamitsu/ability_minamitsu_03.vpcf",
    ...
    local effectIndex2 = ParticleManager:CreateParticle("particles/heroes/minamitsu/ability_minamitsu_03_body.vpcf",
        PATTACH_CUSTOMORIGIN, caster)
...
function OnMinamitsu03AttackLanded(keys)
    ...
    local targets = FindUnitsInRadius(caster:GetTeam(), vecCaster, nil, keys.Radius,
        keys.ability:GetAbilityTargetTeam(), keys.ability:GetAbilityTargetType(), 0, FIND_CLOSEST, false)
    for k, v in pairs(targets) do
        local damage_table = {
            ability = keys.ability,
    ...
    ParticleManager:SetParticleControlTransformForward(effectIndex, 0, vecCaster, forwardVector)
...
function OnMinamitsu04Spawn(keys)
    ...
    local caster = keys.caster
    -- 创建船
    local minamitsu_04_ship = CreateUnitByName("ability_minamitsu_04_ship", caster:GetOrigin(), false, caster, caster,
        caster:GetTeam())
    SetTHD2BlockingNeutrals(minamitsu_04_ship, false)
    minamitsu_04_ship:SetControllableByPlayer(caster:GetPlayerOwnerID(), true)
    FindClearSpaceForUnit(minamitsu_04_ship, minamitsu_04_ship:GetOrigin(), true)
    local particle = "particles/units/heroes/hero_abaddon/abaddon_spawn.vpcf"
    local _particle = ParticleManager:CreateParticle(particle, PATTACH_CUSTOMORIGIN, caster)

    local target = minamitsu_04_ship
...
function OnMinamitsu04ShipDeath(keys)
    ...
    if hero ~= nil and hero:IsNull() == false then
        local ability = hero:FindAbilityByName("ability_thdots_minamitsu04")
        if ability ~= nil then
            -- local boomCooldown = ability:GetCooldownTimeRemaining()
            ability:EndCooldown()
            -- print(hero.boom)
            -- print(boomCooldown)
            if hero.boom then
                ability:StartCooldown(keys.ability:GetCooldown(keys.ability:GetLevel() - 1) / 2)
            else
                ability:StartCooldown(keys.ability:GetCooldown(keys.ability:GetLevel() - 1))
            end
        end
    end
end
        ...
        if ability ~= nil then
            if hero.IsDriving then
                ...
                if FindTelentValue(caster, "special_bonus_unique_minamitsu_5") == 0 then
                    ability:ApplyDataDrivenModifier(hero, hero, "modifier_minamitsu04_ship_destroy", {
                        Duration = keys.DrownDuration
                    })
                end
            end
        end
...
function OnMinamitsu04ShipUpload(caster)
    ...
    caster:SetContextThink(DoUniqueString("ability_thdots_minamitsu_04_move_to_ship"), function()
        if GameRules:IsGamePaused() then
            return 0.03
        end
...
function OnMinamitsu04ShipBack(keys)
    ...
    local effectIndex = ParticleManager:CreateParticle("particles/units/heroes/hero_chen/chen_holy_persuasion_a.vpcf",
        PATTACH_CUSTOMORIGIN, nil)
...
function OnMinamitsu04ShipUnitBoom(keys)
    ...
    caster:SetContextThink(DoUniqueString("ability_thdots_minamitsu_04_unit_ship_destroy"), function()
        if GameRules:IsGamePaused() then
            return 0.03
        end
        ...
        if distance_end >= 0 then
            ...
            local targets = FindUnitsInRadius(hero:GetTeam(), targetPoint, nil, 300,
                keys.ability:GetAbilityTargetTeam(), keys.ability:GetAbilityTargetType(), 0, FIND_CLOSEST, false)
            for k, v in pairs(targets) do
                local damage_table = {
                    ability = keys.ability,
            ...
            local effectIndex = ParticleManager:CreateParticle(
                "particles/heroes/minamitsu/ability_minamitsu_04_boom.vpcf", PATTACH_CUSTOMORIGIN, nil)
