...
function ability_thdots_minoriko01:OnSpellStart()
    ...
    if not self:GetCaster():HasModifier("modifier_item_aghanims_shard") then
        ...
        local targets = FindUnitsInRadius(caster:GetTeam(), target:GetOrigin(), nil, radius,
...
function ability_thdots_minoriko01:OnProjectileHit(hTarget, vLocation)
    ...
    if caster:GetTeam() ~= target:GetTeam() then
        ...
        UtilStun:UnitStunTarget(caster, target,
            (stun_duration + FindTelentValue(caster, "special_bonus_unique_minoriko_3")))
        local targets = FindUnitsInRadius(caster:GetTeam(), target:GetOrigin(), nil, radius,
            DOTA_UNIT_TARGET_TEAM_ENEMY, self:GetAbilityTargetType(), 0, FIND_CLOSEST, false)
    ...
    elseif target:GetUnitName() == "npc_thdots_unit_minoriko02_box" and caster:GetPlayerOwner() ==
        ...
        local minoriko01_box_particle_fx = ParticleManager:CreateParticle(minoriko01_box_particle, PATTACH_ABSORIGIN,
            target)
        ...
        -- 对周围敌方造成伤害
        local damage_targets = FindUnitsInRadius(caster:GetTeam(), target:GetOrigin(), nil, minoriko02_radius,
            DOTA_UNIT_TARGET_TEAM_ENEMY, self:GetAbilityTargetType(), 0, FIND_CLOSEST, false)
        ...
        SendOverheadEventMessage(nil, OVERHEAD_ALERT_HEAL, target,
            heal_amount + target:GetMaxHealth() * heal_percent / 100, nil)
        local effectIndex = ParticleManager:CreateParticle(
            "particles/units/heroes/hero_ogre_magi/ogre_magi_bloodlust_buff.vpcf", PATTACH_CUSTOMORIGIN, target)
        ParticleManager:SetParticleControl(effectIndex, 0, target:GetOrigin())
...
LinkLuaModifier("modifier_ability_thdots_minoriko01_sign", "scripts/vscripts/abilities/abilityMinoriko.lua",
    LUA_MODIFIER_MOTION_NONE)
function modifier_ability_thdots_minoriko01_sign:IsHidden()
    return true
end
...
function OnMinoriko01ProjectileHitUnit(keys)
    ...
    if caster:GetTeam() ~= target:GetTeam() then
        ...
        UtilStun:UnitStunTarget(caster, target,
            (keys.stun_duration + FindTelentValue(caster, "special_bonus_unique_minoriko_3")))
        local effectIndex = ParticleManager:CreateParticle(
            "particles/units/heroes/hero_ogre_magi/ogre_magi_fireblast.vpcf", PATTACH_CUSTOMORIGIN, target)
        ParticleManager:SetParticleControl(effectIndex, 0, target:GetOrigin())
        ParticleManager:SetParticleControl(effectIndex, 1, target:GetOrigin())
        UnitDamageTarget(damage_table)
        local targets = FindUnitsInRadius(caster:GetTeam(), targetLoc, nil, keys.radius, DOTA_UNIT_TARGET_TEAM_ENEMY,
    ...
    elseif target:GetUnitName() == "npc_thdots_unit_minoriko02_box" and caster:GetPlayerOwner() ==
        ...
        -- 对周围友军治疗
        local heal_targets = FindUnitsInRadius(caster:GetTeam(), target:GetOrigin(), nil, minoriko02_radius,
        ...
        SendOverheadEventMessage(nil, OVERHEAD_ALERT_HEAL, target,
            keys.heal_amount + target:GetMaxHealth() * keys.heal_percent / 100, nil)
        local effectIndex = ParticleManager:CreateParticle(
            "particles/units/heroes/hero_ogre_magi/ogre_magi_bloodlust_buff.vpcf", PATTACH_CUSTOMORIGIN, target)
        ParticleManager:SetParticleControl(effectIndex, 0, target:GetOrigin())
        ParticleManager:DestroyParticleSystemTime(effectIndex, 1.0)
...
LinkLuaModifier("modifier_thdots_minoriko02_box", "scripts/vscripts/abilities/abilityminoriko.lua",
    LUA_MODIFIER_MOTION_NONE)
...
function OnMinoriko02SpellStart(keys)
    ...
    local minoriko02 = CreateUnitByName("npc_thdots_unit_minoriko02_box", targetPoint, false, caster, caster,
        caster:GetTeam())
    ...
    minoriko02:AddNewModifier(minoriko02, keys.ability, "modifier_thdots_minoriko02_box", {
        Duration = NewDuration
    })
    if caster:HasModifier("modifier_item_wanbaochui") then
        keys.ability:ApplyDataDrivenModifier(minoriko02, minoriko02, "aura_thdots_minoriko02_wanbaochui", {
            Duration = NewDuration
        })
    end

    minoriko02:SetContextThink(DoUniqueString("OnMinoriko02Destroy"), function()
        ...
        minoriko02:Destroy()
        return nil
    end, keys.duration + FindTelentValue(caster, "special_bonus_unique_minoriko_4"))
end
...
function OnMinoriko02RegenHealth(keys)
    ...
    keys.target:SetContextThink(DoUniqueString("Minoriko02RegenHealth"), function()
        if count < 4 then
...
function modifier_thdots_minoriko02_box:IsHidden()
    return true
end
function modifier_thdots_minoriko02_box:IsPurgable()
    return false
end
...
function OnMinoriko04SpellStart(keys)
    ...
    caster:SetContextThink(DoUniqueString("OnMinoriko04SpellThink"), function()
        if GameRules:IsGamePaused() then
            return 0.03
        end
        ...
        if caster:GetTeam() == target:GetTeam() then
            local targets = FindUnitsInRadius(caster:GetTeam(), target:GetOrigin(), nil,
                keys.radius + FindTelentValue(caster, "special_bonus_unique_minoriko_1"), DOTA_UNIT_TARGET_TEAM_ENEMY,
                keys.ability:GetAbilityTargetType(), 0, FIND_CLOSEST, false)
            if #targets > 0 then
                ...
                target:SetModifierStackCount("aura_thdots_minoriko04_buff", caster,
                    stackCount + keys.strength_amount * (#targets))
                keys.ability:ApplyDataDrivenModifier(caster, target, "aura_thdots_minoriko04_strength", {})

                for k, v in pairs(targets) do

                    local effectIndex = ParticleManager:CreateParticle(
                        "particles/heroes/minoriko/ability_minoriko_04_laser.vpcf", PATTACH_CUSTOMORIGIN, caster)
            ...
            if #targets > 0 then
                ...
                target:SetModifierStackCount("aura_thdots_minoriko04_debuff", caster,
                    stackCount + keys.strength_amount * (#targets))
                keys.ability:ApplyDataDrivenModifier(caster, target, "aura_thdots_minoriko04_strength", {})
