...
function Child01OnProjectileHitUnit(keys)
    ...
    ability:ApplyDataDrivenModifier(caster, target, "modifier_ability_thdots_child01_debuff", {
...
function Child02OnSpellStart(keys)
    ...
    local ability = keys.ability
    Child02targetPoint = keys.ability:GetCursorPosition()
    local effectIndex = ParticleManager:CreateParticle(
        "particles/units/heroes/hero_void_spirit/void_spirit_entryportal.vpcf", PATTACH_WORLDORIGIN, caster)
    ...
    CreateModifierThinker(caster, ability, "modifier_ability_thdots_child02_thinker", {
        duration = duration
    }, Child02targetPoint, caster:GetTeamNumber(), false)
...
function ability_thdots_child01:OnSpellStart()
    ...
    self:SetContextThink("child01Shooting", function()
...
function ability_thdots_child01:OnProjectileHit(Target, Location)
    ...
    if Target:HasModifier("modifier_ability_thdots_child01_debuff") then
        ...
        Target:AddNewModifier(self:GetCaster(), self, "modifier_ability_thdots_child01_debuff", {
    ...
    local damage_table = {
        ...
        damage = self:GetSpecialValueFor("damage") + self:GetCaster():GetAgility() *
            self:GetSpecialValueFor("attack_bonus"),
        damage_type = type,
        damage_flags = 0
    }
    UnitDamageTarget(damage_table)
...
LinkLuaModifier("modifier_ability_thdots_child01_debuff", "scripts/vscripts/abilities/abilitychild.lua",
    LUA_MODIFIER_MOTION_NONE)
function modifier_ability_thdots_child01_debuff:IsHidden()
    return false
end
function modifier_ability_thdots_child01_debuff:IsPurgable()
    return true
end
function modifier_ability_thdots_child01_debuff:RemoveOnDeath()
    return true
end
function modifier_ability_thdots_child01_debuff:IsDebuff()
    return true
end
...
LinkLuaModifier("modifier_ability_thdots_child02_thinker", "scripts/vscripts/abilities/abilitychild.lua",
    LUA_MODIFIER_MOTION_NONE)
function modifier_ability_thdots_child02_thinker:IsHidden()
    return false
end
function modifier_ability_thdots_child02_thinker:IsPurgable()
    return false
end
function modifier_ability_thdots_child02_thinker:RemoveOnDeath()
    return false
end
function modifier_ability_thdots_child02_thinker:IsDebuff()
    return false
end
...
function modifier_ability_thdots_child02_thinker:OnAbilityExecuted(keys)
    ...
    if keys.ability:IsItem() or IsNotLunchbox_ability(keys.ability) then
        return
    end
    if target:GetTeam() ~= caster:GetTeam() and (target:GetOrigin() - self.point):Length2D() <= self.radius then
        target:AddNewModifier(caster, self.ability, "modifier_ability_thdots_child02_thinker_debuff", {
...
function modifier_ability_thdots_child02_thinker:OnIntervalThink()
    ...
    for _, v in pairs(targets) do
        v:AddNewModifier(caster, self.ability, "modifier_ability_thdots_child02_thinker_sign", {
            duration = 0.2
        })
    end
end
...
LinkLuaModifier("modifier_ability_thdots_child02_thinker_sign", "scripts/vscripts/abilities/abilitychild.lua",
    LUA_MODIFIER_MOTION_NONE)
function modifier_ability_thdots_child02_thinker_sign:IsHidden()
    return true
end
function modifier_ability_thdots_child02_thinker_sign:IsPurgable()
    return false
end
function modifier_ability_thdots_child02_thinker_sign:RemoveOnDeath()
    return false
end
function modifier_ability_thdots_child02_thinker_sign:IsDebuff()
    return true
end

modifier_ability_thdots_child02_thinker_debuff = {}
LinkLuaModifier("modifier_ability_thdots_child02_thinker_debuff", "scripts/vscripts/abilities/abilitychild.lua",
    LUA_MODIFIER_MOTION_NONE)
function modifier_ability_thdots_child02_thinker_debuff:IsHidden()
    return false
end
function modifier_ability_thdots_child02_thinker_debuff:IsPurgable()
    return false
end
function modifier_ability_thdots_child02_thinker_debuff:RemoveOnDeath()
    return false
end
function modifier_ability_thdots_child02_thinker_debuff:IsDebuff()
    return true
end
...
function modifier_ability_thdots_child02_thinker_debuff:OnCreated()
    ...
    target:AddNewModifier(caster, ability, "modifier_silence", -- 沉默修饰器
    {
        duration = silence_duration * (1 - target:GetStatusResistance())
    })
    target:EmitSound("Hero_Wisp.Spirits.Target")
    local effectIndex = ParticleManager:CreateParticle(
        "particles/econ/items/storm_spirit/strom_spirit_ti8/storm_sprit_ti8_overload_discharge.vpcf", PATTACH_POINT,
        target)
...
function ability_thdots_child03:CreateNewLight(location)
    ...
    local light = CreateUnitByName("npc_ability_child_03_light", location, false, caster, caster, caster:GetTeam())
    light:AddNewModifier(caster, self, "modifier_ability_thdots_child03_light", {
        duration = 5
    })
end

function ability_thdots_child03:GetIntrinsicModifierName()
    return "modifier_ability_thdots_child03"
end

modifier_ability_thdots_child03 = {}
LinkLuaModifier("modifier_ability_thdots_child03", "scripts/vscripts/abilities/abilitychild.lua",
    LUA_MODIFIER_MOTION_NONE)
function modifier_ability_thdots_child03:IsHidden()
    return true
end
function modifier_ability_thdots_child03:IsPurgable()
    return false
end
function modifier_ability_thdots_child03:RemoveOnDeath()
    return false
end
function modifier_ability_thdots_child03:IsDebuff()
    return false
end
...
LinkLuaModifier("modifier_ability_thdots_child03_light", "scripts/vscripts/abilities/abilitychild.lua",
    LUA_MODIFIER_MOTION_NONE)
function modifier_ability_thdots_child03_light:IsHidden()
    return true
end
function modifier_ability_thdots_child03_light:IsPurgable()
    return false
end
function modifier_ability_thdots_child03_light:RemoveOnDeath()
    return false
end
function modifier_ability_thdots_child03_light:IsDebuff()
    return false
end
...
function modifier_ability_thdots_child03_light:OnIntervalThink()
    ...
    local targets_near = FindUnitsInRadius(caster:GetTeam(), self.location, nil, 500, ability:GetAbilityTargetTeam(),
    ...
    elseif self.t < math.pi * 2 then -- 转完了冲目标而去
        vec = Vector(vec.x + (math.cos(rad) + math.cos(self.t)) * (20 - math.pi * 2 + self.t),
            vec.y + (math.sin(rad) + math.sin(self.t)) * (20 - math.pi * 2 + self.t), vec.z - 1)
        light:SetAbsOrigin(vec)
    elseif self.t > math.pi * 2 then
        ...
        if GetDistanceBetweenTwoVec2D(light:GetAbsOrigin(), target:GetAbsOrigin()) < 50 then -- 击中距离判定
            ...
            if not target:IsBuilding() then
                local child_03_debuff = target:FindModifierByNameAndCaster("modifier_ability_thdots_child03_debuff",
                    caster)
                ...
                if not child_03_debuff then
                    ...
                    if count < debuff_max then -- 层数上限
                        target:SetModifierStackCount("modifier_ability_thdots_child03_debuff", ability,
                            count + debuff_stack)
                    end
                end
...
LinkLuaModifier("modifier_ability_thdots_child03_debuff", "scripts/vscripts/abilities/abilitychild.lua",
    LUA_MODIFIER_MOTION_NONE)
function modifier_ability_thdots_child03_debuff:IsHidden()
    return false
end
function modifier_ability_thdots_child03_debuff:IsPurgable()
    return true
end
function modifier_ability_thdots_child03_debuff:RemoveOnDeath()
    return true
end
function modifier_ability_thdots_child03_debuff:IsDebuff()
    return true
end
...
function ability_thdots_child04:OnSpellStart()
    ...
    local distance = (vector_distance):Length2D()
    local direction = (vector_distance):Normalized()
    local targets = FindUnitsInLine(caster:GetTeam(), vec_caster, vec_caster + direction * range, nil, radius,
        self:GetAbilityTargetTeam(), self:GetAbilityTargetType(), 0)
    local num = 100
    caster:EmitSound("Hero_FacelessVoid.Chronosphere")
    for i = 0, range / num do
        local dummy = CreateUnitByName("npc_dummy_unit", vec_caster + direction * num * i, false, caster, caster,
            caster:GetTeam())
        local ability_dummy_unit = dummy:FindAbilityByName("ability_dummy_unit")
        ability_dummy_unit:SetLevel(1)
        local effectIndex = ParticleManager:CreateParticle(
            "particles/econ/items/antimage/antimage_weapon_basher_ti5_gold/antimage_manavoid_explode_b_ti_5_gold.vpcf",
    ...
    if #targets > 0 then
        caster:AddNewModifier(caster, self, "modifier_ability_thdots_child04", {
            duration = duration
        })
...
LinkLuaModifier("modifier_ability_thdots_child04", "scripts/vscripts/abilities/abilitychild.lua",
    LUA_MODIFIER_MOTION_NONE)
function modifier_ability_thdots_child04:IsHidden()
    return false
end
function modifier_ability_thdots_child04:IsPurgable()
    return false
end
function modifier_ability_thdots_child04:RemoveOnDeath()
    return true
end
function modifier_ability_thdots_child04:IsDebuff()
    return false
end
...
LinkLuaModifier("modifier_ability_thdots_childEx", "scripts/vscripts/abilities/abilitychild.lua",
    LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_ability_thdots_childEx_blur", "scripts/vscripts/abilities/abilitychild.lua",
    LUA_MODIFIER_MOTION_NONE)

function modifier_ability_thdots_childEx:IsHidden()
    return false
end
function modifier_ability_thdots_childEx:IsPurgable()
    return false
end
function modifier_ability_thdots_childEx:RemoveOnDeath()
    return false
end
function modifier_ability_thdots_childEx:IsDebuff()
    return false
end
...
function modifier_ability_thdots_childEx:OnIntervalThink()
    if IsServer() then
        ...
        -- Find nearby enemies
        local nearby_enemies = FindUnitsInRadius(self.caster:GetTeamNumber(), self.caster:GetAbsOrigin(), nil,
            self.radius, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES,
...
function modifier_ability_thdots_childEx_blur:IsHidden()
    return true
end
function modifier_ability_thdots_childEx_blur:IsDebuff()
    return false
end
function modifier_ability_thdots_childEx_blur:IsPurgable()
    return false
end
function modifier_ability_thdots_childEx_blur:StatusEffectPriority()
    return 11
end
