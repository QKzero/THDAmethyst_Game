...
function OnMokouKill(keys)
    ...
    if caster:HasModifier("modifier_item_wanbaochui") and keys.unit:IsHero() == true and
        keys.unit:HasModifier("modifier_illusion") == false then
...
function OnMokou01SpellMove(keys)
    ...
    if (keys.ability.ability_Mokou01_Distance < 30) then
        ...
        local effectIndex = ParticleManager:CreateParticle("particles/heroes/mouko/ability_mokou_01_boom.vpcf",
        ...
        local vec = Vector(vecCaster.x + math.cos(Mokou01rad) * keys.MoveSpeed / 50,
            vecCaster.y + math.sin(Mokou01rad) * keys.MoveSpeed / 50, GetGroundPosition(vecCaster, nil).z)
...
function OnMokou02DamageStart(keys)
    ...
    local effectIndex = ParticleManager:CreateParticle("particles/thd2/heroes/mouko/ability_mokou_02_boom.vpcf",
        PATTACH_CUSTOMORIGIN, caster)
...
function OnMokou04SpellStart(keys)
    ...
    Timer.Wait 'ability_Mokou02_damage_bouns_timer'(20, function()
        caster.ability_Mokou02_damage_bouns = 0
    end)

    local effectIndex = ParticleManager:CreateParticle("particles/thd2/heroes/mouko/ability_mokou_04_wing.vpcf",
        PATTACH_CUSTOMORIGIN, caster)
    ParticleManager:SetParticleControlEnt(effectIndex, 0, caster, 5, "follow_origin", Vector(0, 0, 0), true)

    Timer.Wait 'ability_mokou_04_wing_destory'(20, function()
...
function ability_thdots_mokou03:OnSpellStart()
    ...
    egg:SetMaxHealth(hp)
    egg:SetHealth(hp)
    egg:AddNewModifier(caster, self, "modifier_phenx_egg_form", {
        Duration = duration
    })
...
function modifier_phenx_egg_form:OnCreated(table)
    if IsServer() then
        self.nfx = ParticleManager:CreateParticle("particles/units/heroes/hero_phoenix/phoenix_supernova_egg.vpcf",
            PATTACH_POINT_FOLLOW, self:GetParent())
        ParticleManager:SetParticleControlEnt(self.nfx, 0, self:GetParent(), PATTACH_POINT_FOLLOW, "attach_hitloc",
            self:GetParent():GetAbsOrigin(), false)
        ParticleManager:SetParticleControlEnt(self.nfx, 1, self:GetParent(), PATTACH_POINT_FOLLOW, "attach_hitloc",
            self:GetParent():GetAbsOrigin(), false)
...
function modifier_phenx_egg_form:DeclareFunctions()
    return {MODIFIER_PROPERTY_DISABLE_HEALING, MODIFIER_PROPERTY_INCOMING_DAMAGE_PERCENTAGE,
...
function modifier_phenx_egg_form:GetModifierIncomingDamage_Percentage(params)
    local form_particel = ParticleManager:CreateParticle(
        "particles/units/heroes/hero_phoenix/phoenix_supernova_hit.vpcf", PATTACH_POINT, params.target)
...
function modifier_phenx_egg_form:OnRemoved()
    if IsServer() then
        ...
        local pfxName = "particles/units/heroes/hero_phoenix/phoenix_supernova_" .. (isDead and "death" or "reborn") ..
                            ".vpcf"
        local pfx = ParticleManager:CreateParticle(pfxName, PATTACH_ABSORIGIN, caster)
        ParticleManager:SetParticleControlEnt(pfx, 0, caster, PATTACH_POINT_FOLLOW, "follow_origin",
            caster:GetAbsOrigin(), true)
        ParticleManager:SetParticleControlEnt(pfx, 1, caster, PATTACH_POINT_FOLLOW, "attach_hitloc",
            caster:GetAbsOrigin(), true)
