...
function refresh_mod:IsHidden()
    return false
end -- 是否隐藏
function refresh_mod:IsPurgable()
    return false
end -- 是否可净化
function refresh_mod:RemoveOnDeath()
    return false
end -- 死亡移除
function refresh_mod:IsDebuff()
    return false
end -- 是否是DEBUFF
...
function ability_thdots_kasenEx:OnSpellStart()
    ...
    if not caster:HasModifier("modifier_thdots_kasen_ex") then
        caster:AddNewModifier(caster, self, "modifier_thdots_kasen_ex", {
            duration = duration
        })
        self:EndCooldown()
    else
    ...
    caster:AddNewModifier(self:GetCaster(), self, "refresh_mod", {
        duration = 0.03
    })
end
...
function modifier_thdots_kasen_ex:IsHidden()
    return false
end -- 是否隐藏
function modifier_thdots_kasen_ex:IsPurgable()
    return false
end -- 是否可净化
function modifier_thdots_kasen_ex:RemoveOnDeath()
    return false
end -- 死亡移除
function modifier_thdots_kasen_ex:IsDebuff()
    return false
end -- 是否是DEBUFF
...
function modifier_thdots_kasen_ex:OnCreated()
    ...
    self.lock = 0
    self.particle = ParticleManager:CreateParticle("models/ibaraki_kasen/ibaraki_kasen2_ambient.vpcf",
    ...
    local effectIndex = ParticleManager:CreateParticle(
        "particles/econ/events/spring_2021/blink_dagger_spring_2021_start_lvl2.vpcf", PATTACH_CUSTOMORIGIN, nil)
...
function modifier_thdots_kasen_ex:OnRemoved()
    ...
    ability = caster:FindAbilityByName("ability_thdots_kasen03")
    ability:SetActivated(true)
    caster:AddNewModifier(caster, self, "refresh_mod", {
        duration = 0.03
    })
end
...
LinkLuaModifier("modifier_thdots_kasen_ex_WBC_check", "scripts/vscripts/abilities/abilitykasen.lua",
    LUA_MODIFIER_MOTION_NONE)
function modifier_thdots_kasen_ex_WBC_check:IsHidden()
    return true
end
function modifier_thdots_kasen_ex_WBC_check:IsPurgable()
    return false
end
function modifier_thdots_kasen_ex_WBC_check:RemoveOnDeath()
    return false
end
function modifier_thdots_kasen_ex_WBC_check:IsDebuff()
    return false
end
...
function modifier_thdots_kasen_ex_WBC_check:OnIntervalThink()
    ...
    if FindTelentValue(caster, "special_bonus_unique_kasen_6") ~= 0 and
        not caster:HasModifier("modifier_thdots_kasen_ex_WBC") then
        ...
        caster:AddNewModifier(caster, self:GetAbility(), "modifier_thdots_kasen_ex_WBC", {
            duration = -1
        })
    end
end
...
function modifier_thdots_kasen_ex_WBC:IsHidden()
    return false
end -- 是否隐藏
function modifier_thdots_kasen_ex_WBC:IsPurgable()
    return false
end -- 是否可净化
function modifier_thdots_kasen_ex_WBC:RemoveOnDeath()
    return false
end -- 死亡移除
function modifier_thdots_kasen_ex_WBC:IsDebuff()
    return false
end -- 是否是DEBUFF
...
function modifier_thdots_kasen_ex_WBC:OnCreated()
    ...
    self.particle = ParticleManager:CreateParticle("models/ibaraki_kasen/ibaraki_kasen3_ambient.vpcf",
        PATTACH_ABSORIGIN_FOLLOW, caster)
...
function modifier_thdots_kasen_ex_WBC:DeclareFunctions()
    return {MODIFIER_EVENT_ON_ABILITY_FULLY_CAST, MODIFIER_PROPERTY_INCOMING_DAMAGE_PERCENTAGE}
end
...
function modifier_thdots_kasen_ex_WBC:OnAbilityFullyCast(keys)
    ...
    if Parent ~= keys.unit or keys.ability:IsItem() then
        return
    end
...
function Kasen04_OnUpgrade(keys)
    ...
    if caster:GetClassname() ~= "npc_dota_hero_bristleback" then
        return
    end
    if caster.upgrading == nil then
...
function Kasen02_OnUpgrade(keys)
    local caster = keys.caster
    if caster:HasModifier("modifier_thdots_kasen_ex") == false and caster:HasModifier("modifier_thdots_kasen_ex_WBC") ==
        false then
...
function ability_thdots_kasen01:OnSpellStart()
    ...
    local buff_duration = self:GetSpecialValueFor("buff_duration") +
                              FindTelentValue(caster, "special_bonus_unique_kasen_1")
    local time = 0
    ...
    GameRules:GetGameModeEntity():SetContextThink(DoUniqueString("kasen01"), function()
        ...
        local targets = FindUnitsInRadius(caster:GetTeam(), -- team
        ...
        for _, v in pairs(targets) do
            if v:IsRealHero() then
                -- 增加力量
                caster:AddNewModifier(caster, self, "modifier_thdots_kasen01_buff_strength", {
                    duration = buff_duration
                })
                -- 减少力量
                v:AddNewModifier(caster, self, "modifier_thdots_kasen01_debuff_strength", {
                    duration = buff_duration
                })
            end
            local damage_table = {
    ...
    local time02 = 0
    GameRules:GetGameModeEntity():SetContextThink(DoUniqueString("kasen01move"), function()
        if GameRules:IsGamePaused() then
            return 0.03
        end
...
LinkLuaModifier("modifier_thdots_kasen01_Passive", "scripts/vscripts/abilities/abilitykasen.lua",
    LUA_MODIFIER_MOTION_NONE)
function modifier_thdots_kasen01_Passive:IsHidden()
    return true
end -- 是否隐藏
function modifier_thdots_kasen01_Passive:IsPurgable()
    return false
end -- 是否可净化
function modifier_thdots_kasen01_Passive:RemoveOnDeath()
    return false
end -- 死亡移除
function modifier_thdots_kasen01_Passive:IsDebuff()
    return false
end -- 是否是DEBUFF
...
function modifier_thdots_kasen01_Passive:OnAttackLanded(keys)
    ...
    local chance = self:GetAbility():GetSpecialValueFor("chance") +
                       FindTelentValue(caster, "special_bonus_unique_kasen_3")
    if target:IsRealHero() and RandomInt(0, 100) < chance then
        -- 有天生状态时继续
        if caster:HasModifier("modifier_thdots_kasen_ex") == false and
            caster:HasModifier("modifier_thdots_kasen_ex_WBC") == false then
            return
        end
        caster:EmitSound("Hero_Centaur.DoubleEdge.TI9")
        caster:AddNewModifier(caster, ability, "modifier_thdots_kasen01_buff_strength_Passive", {
            duration = buff_duration
        })
        target:AddNewModifier(caster, ability, "modifier_thdots_kasen01_debuff_strength_Passive", {
            duration = buff_duration
        })
    end
end
modifier_thdots_kasen01_buff_strength = {}
LinkLuaModifier("modifier_thdots_kasen01_buff_strength", "scripts/vscripts/abilities/abilitykasen.lua",
    LUA_MODIFIER_MOTION_NONE)
function modifier_thdots_kasen01_buff_strength:IsHidden()
    return true
end -- 是否隐藏
function modifier_thdots_kasen01_buff_strength:IsPurgable()
    return false
end -- 是否可净化
function modifier_thdots_kasen01_buff_strength:RemoveOnDeath()
    return false
end -- 死亡移除
function modifier_thdots_kasen01_buff_strength:IsDebuff()
    return false
end -- 是否是DEBUFF
function modifier_thdots_kasen01_buff_strength:GetAttributes()
    return MODIFIER_ATTRIBUTE_MULTIPLE
end
function modifier_thdots_kasen01_buff_strength:DeclareFunctions()
    return {MODIFIER_PROPERTY_STATS_STRENGTH_BONUS}
end
...
LinkLuaModifier("modifier_thdots_kasen01_debuff_strength", "scripts/vscripts/abilities/abilitykasen.lua",
    LUA_MODIFIER_MOTION_NONE)
function modifier_thdots_kasen01_debuff_strength:IsHidden()
    return true
end -- 是否隐藏
function modifier_thdots_kasen01_debuff_strength:IsPurgable()
    return false
end -- 是否可净化
function modifier_thdots_kasen01_debuff_strength:RemoveOnDeath()
    return false
end -- 死亡移除
function modifier_thdots_kasen01_debuff_strength:IsDebuff()
    return true
end -- 是否是DEBUFF
function modifier_thdots_kasen01_debuff_strength:GetAttributes()
    return MODIFIER_ATTRIBUTE_MULTIPLE
end
function modifier_thdots_kasen01_debuff_strength:DeclareFunctions()
    return {MODIFIER_PROPERTY_STATS_STRENGTH_BONUS}
end
...
LinkLuaModifier("modifier_thdots_kasen01_buff_strength_Passive", "scripts/vscripts/abilities/abilitykasen.lua",
    LUA_MODIFIER_MOTION_NONE)
function modifier_thdots_kasen01_buff_strength_Passive:IsHidden()
    return true
end -- 是否隐藏
function modifier_thdots_kasen01_buff_strength_Passive:IsPurgable()
    return false
end -- 是否可净化
function modifier_thdots_kasen01_buff_strength_Passive:RemoveOnDeath()
    return false
end -- 死亡移除
function modifier_thdots_kasen01_buff_strength_Passive:IsDebuff()
    return false
end -- 是否是DEBUFF
function modifier_thdots_kasen01_buff_strength_Passive:GetAttributes()
    return MODIFIER_ATTRIBUTE_MULTIPLE
end
function modifier_thdots_kasen01_buff_strength_Passive:DeclareFunctions()
    return {MODIFIER_PROPERTY_STATS_STRENGTH_BONUS}
end
...
LinkLuaModifier("modifier_thdots_kasen01_debuff_strength_Passive", "scripts/vscripts/abilities/abilitykasen.lua",
    LUA_MODIFIER_MOTION_NONE)
function modifier_thdots_kasen01_debuff_strength_Passive:IsHidden()
    return true
end -- 是否隐藏
function modifier_thdots_kasen01_debuff_strength_Passive:IsPurgable()
    return false
end -- 是否可净化
function modifier_thdots_kasen01_debuff_strength_Passive:RemoveOnDeath()
    return false
end -- 死亡移除
function modifier_thdots_kasen01_debuff_strength_Passive:IsDebuff()
    return true
end -- 是否是DEBUFF
function modifier_thdots_kasen01_debuff_strength_Passive:GetAttributes()
    return MODIFIER_ATTRIBUTE_MULTIPLE
end
function modifier_thdots_kasen01_debuff_strength_Passive:DeclareFunctions()
    return {MODIFIER_PROPERTY_STATS_STRENGTH_BONUS}
end
...
function Kasen02PassiveKnock(keys)
    ...
    local ability = keys.ability
    if caster:HasModifier("modifier_thdots_kasen_ex") == true and caster:HasModifier("modifier_thdots_kasen_ex_WBC") ==
        false then
        return
    end
    ...
    local effectIndex = ParticleManager:CreateParticle("particles/units/heroes/hero_tusk/tusk_walruspunch_start.vpcf",
...
function OnKasen02ProjectileHitUnit(keys)
    ...
    for _, v in pairs(targets) do
        ...
        UnitDamageTarget(damage_tabel)
        ability:ApplyDataDrivenModifier(caster, v, "modifier_thdots_kasen02_slow", {
...
function OnKasen03SpellStart(keys)
    ...
    local interval_damage = ability:GetSpecialValueFor("interval_damage") + caster:GetStrength() *
                                ability:GetSpecialValueFor("strength_multiply")
    ...
    GameRules:GetGameModeEntity():SetContextThink(DoUniqueString("interval_flash_ball"), function()
        if GameRules:IsGamePaused() then
            return 0.03
        end
        if time <= fTime then
            ...
            local effectIndex = ParticleManager:CreateParticle("particles/heroes/kasen/kasen03_land.vpcf",
                PATTACH_CUSTOMORIGIN, nil)
...
function OnKasen03ProjectileHitUnit(keys)
    ...
    keys.ability:ApplyDataDrivenModifier(caster, target, "modifier_thdots_kasen03_slow", {
        duration = duration
    })
    local damage_tabel = {
        victim = target,
...
function Kasen03Passive(keys)
    ...
    if caster:HasModifier("modifier_thdots_kasen03_cd") then
        return
    end
    ...
    local passive_damage = ability:GetSpecialValueFor("passive_damage") + caster:GetStrength() *
                               ability:GetSpecialValueFor("strength_multiply")
    ...
    -- 最短间隔
    if caster:HasModifier("passive_kasen03_banned") then
        return
    end
    local banned_duration = ability:GetSpecialValueFor("banned_duration")
    ability:ApplyDataDrivenModifier(caster, caster, "passive_kasen03_banned", {
        duration = banned_duration
    })
    ...
    if target:HasModifier("passive_kasen03_check") then
        ...
        ParticleManager:SetParticleControl(lightningBolt, 0, Vector(caster:GetAbsOrigin().x, caster:GetAbsOrigin().y,
            caster:GetAbsOrigin().z + caster:GetBoundingMaxs().z))
        ParticleManager:SetParticleControl(lightningBolt, 1, Vector(target:GetAbsOrigin().x, target:GetAbsOrigin().y,
    ...
    for _, v in pairs(targets) do
        if targets_shocked < max_targets then
            if v ~= target and v:HasModifier("passive_kasen03_check") then
                ...
                local lightningBolt = ParticleManager:CreateParticle(particleName, PATTACH_WORLDORIGIN, caster)
                ParticleManager:SetParticleControl(lightningBolt, 0, Vector(caster:GetAbsOrigin().x,
                    caster:GetAbsOrigin().y, caster:GetAbsOrigin().z + caster:GetBoundingMaxs().z))
                ParticleManager:SetParticleControl(lightningBolt, 1,
                    Vector(origin.x, origin.y, origin.z + v:GetBoundingMaxs().z))
...
function Kasen03PassiveOnAbility(keys)
    ...
    if target:GetTeam() ~= caster:GetTeam() and TypeTarget == true then
        ...
        if target:HasModifier("passive_kasen03_check") then
            ...
            ParticleManager:SetParticleControl(lightningBolt, 0, Vector(caster:GetAbsOrigin().x,
                caster:GetAbsOrigin().y, caster:GetAbsOrigin().z + caster:GetBoundingMaxs().z))
            ParticleManager:SetParticleControl(lightningBolt, 1, Vector(target:GetAbsOrigin().x,
                target:GetAbsOrigin().y, target:GetAbsOrigin().z + target:GetBoundingMaxs().z))
...
function OnKasen04Normal(event)
    ...
    ParticleManager:DestroyParticleSystemTime(pfx, duration)
    ...
    ParticleManager:ReleaseParticleIndex(pfx)
    ParticleManager:DestroyParticleSystemTime(pfx, stun_duration)
    ...
    for i = 1, numProjectiles do
        ...
        ability:ApplyDataDrivenThinker(caster, projectilePos, "modifier_thdots_kasen04normal_debuff_aura", {
            duration = duration
        })
        local targets = FindUnitsInRadius(caster:GetTeam(), -- team
        ...
        for _, v in pairs(targets) do
            ability:ApplyDataDrivenModifier(caster, v, "modifier_kasen04normal_stun", {
...
function OnKasen04ExStart(keys)
    ...
    local time = 3.0
    GameRules:GetGameModeEntity():SetContextThink(DoUniqueString("kasen04ex_count_down"), function()
        if GameRules:IsGamePaused() then
            return 0.03
        end
        if time >= 0 then
...
function OnKasen04ExTakeDamage(keys)
    ...
    local ability = keys.ability
    if IsKilledByYugi04(caster, damage_taken) then
        return
    end
    if caster:HasModifier("modifier_thdots_medicine04_damage") and
        (not caster:HasModifier("modifier_thdots_medicine04_debuff")) and
        caster:HasModifier("modifier_thdots_yugi04_think_interval") then
        return
    end
    ...
    local attack_speed
    local strength
    if ability.kasen04ex_absorbed == nil then
    ...
    if caster:FindModifierByName("modifier_thdots_kasen04ex_strength") ~= nil then
        true_strength = caster:GetStrength() -
                            (caster:FindModifierByName("modifier_thdots_kasen04ex_strength")):GetStackCount()
    else
        true_strength = caster:GetStrength()
    end

    local attack_speed_modifier = ability:ApplyDataDrivenModifier(caster, caster,
    ...
    local strength_modifier = ability:ApplyDataDrivenModifier(caster, caster, "modifier_thdots_kasen04ex_strength", {
        duration = buff_duration
    })
        ...
        if check then
            if not target:HasModifier(modifier_slow) then
                ability:ApplyDataDrivenModifier(caster, target, modifier_slow, {
                    Duration = caster:FindModifierByName("modifier_thdots_kasen04ex_WBC"):GetRemainingTime()
                })
            end
            ability:ApplyDataDrivenModifier(caster, target, modifier_facing, {
                Duration = 0.06
            })
        else
...
function StoneGazeFacing(keys)
    ...
    if target.stone_gaze_look == nil then
        target.stone_gaze_look = 0
    end
    ...
    if target.stone_gaze_look >= face_duration then
        ability:ApplyDataDrivenModifier(caster, target, modifier_stone, {
...
function modifier_thdots_kasen03_cd:IsHidden()
    return true
end
function modifier_thdots_kasen03_cd:IsDebuff()
    return false
end
function modifier_thdots_kasen03_cd:IsPurgable()
    return false
end
