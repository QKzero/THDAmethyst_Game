	...
	GameRules:GetGameModeEntity():SetContextThink(DoUniqueString("OnPatchouliWaterMetal"), 
		function ()
			...
			for _,v in pairs(targets) do
				...
				v:AddNewModifier(caster,keys.ability,"modifier_patchouli_water_metal_debuff",{duration=1.0})
				UnitDamageTarget(damage_table)				
			end
			return 0.5
		end,
...
LinkLuaModifier("modifier_patchouli_water_metal_debuff","scripts/vscripts/abilities/abilitypatchouli.lua",LUA_MODIFIER_MOTION_NONE)
modifier_patchouli_water_metal_debuff = {}

function modifier_patchouli_water_metal_debuff:IsPurgable() 		 return true end
function modifier_patchouli_water_metal_debuff:IsPurgeException()	 return false end
function modifier_patchouli_water_metal_debuff:RemoveOnDeath()		 return false end
function modifier_patchouli_water_metal_debuff:IsHidden()			 return false end
function modifier_patchouli_water_metal_debuff:IsDebuff()			 return true end
function modifier_patchouli_water_metal_debuff:GetAttributes() 	 return MODIFIER_ATTRIBUTE_IGNORE_INVULNERABLE end
			...
			function () 
				...
				for _,v in pairs(enemies) do
					if v:HasModifier("modifier_thdots_patchouli_water_earth_lock")==false then 
						...
						Ability:ApplyDataDrivenModifier(Caster, v, "modifier_thdots_patchouli_water_earth_lock", {duration=(keys.range+keys.ExRange*7)/keys.speed})
						UnitDamageTarget(damage_table)
					end
				end
...
function OnPatchouliXianzhezhishi(keys)
	...
	if caster:HasModifier("modifier_thdots_patchouli_xianzhezhishi_add_cd") then return end
	...
	if FindTelentValue(caster,"special_bonus_unique_patchouli_6") == 1 then
		ability_xianzhezhishi:ApplyDataDrivenModifier(caster, caster, "modifier_thdots_patchouli_xianzhezhishi_add", {Duration = 4.75})
	else
		ability_xianzhezhishi:ApplyDataDrivenModifier(caster, caster, "modifier_thdots_patchouli_xianzhezhishi_add", {Duration = 3.25})
	end
	caster:SetModifierStackCount("modifier_thdots_patchouli_xianzhezhishi_add", ability_xianzhezhishi, caster.stone_count)
	ability_xianzhezhishi:ApplyDataDrivenModifier(caster, caster, "modifier_thdots_patchouli_xianzhezhishi_add_cd", {Duration = 0.25})
