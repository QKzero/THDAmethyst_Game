...
function modifier_thdots_youmu04_states:GetAbsoluteNoDamagePhysical() return 1 end
function modifier_thdots_youmu04_states:GetAbsoluteNoDamageMagical() return 1 end
function modifier_thdots_youmu04_states:GetAbsoluteNoDamagePure() return 1 end
...
function modifier_thdots_youmu04_states:OnCreated()
  ...
  self:StartIntervalThink(0.1)
	...
	GridNav:DestroyTreesAroundPoint(target:GetAbsOrigin(), self.WanbaochuiRadius, false)
...
function ability_thdots_youmu01:GetAOERadius()
	return self:GetSpecialValueFor("damage_radius")
end

function ability_thdots_youmu01:GetCastRange(location, target)
	if IsServer() then return 0 end
end

function ability_thdots_youmu01:OnSpellStart()
	if not IsServer() then return end
	local caster = self:GetCaster()
	local targetPoint = self:GetCursorPosition()
	local vecCaster = caster:GetOrigin()
	local maxRange = self:GetSpecialValueFor("AbilityCastRange") + caster:GetCastRangeBonus()
	...
	self:SetContextNum("ability_Youmu01_Rad",Youmu01rad,0)
	self:SetContextNum("ability_Youmu01_Move_Speed",Youmu01MoveSpeed,0)
	self:SetContextNum("ability_Youmu01_Count",0,0)

	local move_duration = self:GetSpecialValueFor("move_duration")
	caster:AddNewModifier(caster, self, "modifier_thdots_youmu01_think_interval", {duration = move_duration})
end

modifier_thdots_youmu01_think_interval = {}
LinkLuaModifier("modifier_thdots_youmu01_think_interval", "scripts/vscripts/abilities/abilityyoumu.lua", LUA_MODIFIER_MOTION_NONE)

function modifier_thdots_youmu01_think_interval:IsHidden() return false end
function modifier_thdots_youmu01_think_interval:IsPurgable() return false end
function modifier_thdots_youmu01_think_interval:RemoveOnDeath() return true end
function modifier_thdots_youmu01_think_interval:IsDebuff() return false end
function modifier_thdots_youmu01_think_interval:GetAttributes()
	return MODIFIER_ATTRIBUTE_IGNORE_INVULNERABLE
end
function modifier_thdots_youmu01_think_interval:DeclareFunctions()
	return{
		MODIFIER_PROPERTY_OVERRIDE_ANIMATION
	}
end

function modifier_thdots_youmu01_think_interval:GetOverrideAnimation()
	return ACT_DOTA_CAST_ABILITY_1
end

function modifier_thdots_youmu01_think_interval:CheckState()
	return{
		[MODIFIER_STATE_NO_UNIT_COLLISION] = true,
	}
end

function modifier_thdots_youmu01_think_interval:OnCreated()
	if not IsServer() then return end
	self:StartIntervalThink(0.199)			--神秘数字
end

function modifier_thdots_youmu01_think_interval:OnDestroy()
	if not IsServer() then return end
end

function modifier_thdots_youmu01_think_interval:OnIntervalThink()
	if not IsServer() then return end
	local caster = self:GetCaster()
	local vecCaster = caster:GetOrigin()
	...
	local damage = ability:GetAbilityDamage()+Damage*0.8
	...
	local count = ability:GetContext("ability_Youmu01_Count")
	count = count + 0.2
	if(count == 0.2)then
	    -- Ñ­»µ¸÷¸öÄ¿±êµ¥Î»
		...
		ability:SetContextNum("ability_Youmu01_Count",0,0)
	...
	local Youmu01rad = ability:GetContext("ability_Youmu01_Rad")
	local Youmu01MoveSpeed = ability:GetContext("ability_Youmu01_Move_Speed")
...
function AbilityYoumu:OnYoumu03Start(keys)
	...
	unit:SetBaseAttackTime(caster:GetBaseAttackTime() / caster:GetAttackSpeed(false) * unit:GetAttackSpeed(false))
...
function AbilityYoumu:OnYoumu04Think(keys)
	...
	if(count>=10)then
		...
		caster:RemoveModifierByName("modifier_thdots_youmu04_think_interval")
		return
	end
end
