...
function modifier_ability_thdots_hinaEx_passive:RemoveOnDeath() 	return true end
...
function modifier_ability_thdots_hinaEx_shield:GetModifierIncomingDamageConstant()
	if IsClient() then
...
function modifier_ability_thdots_hinaEx_shield:GetModifierTotal_ConstantBlock(kv)
	...
	self:SetStackCount(self.shield_remaining)
	--只要护盾值小于生命百分比，就开始转CD
	local passive_modifier = caster:FindModifierByName("modifier_ability_thdots_hinaEx_passive")
	-- print("只要护盾值小于生命百分比，就开始转CD")
...
function modifier_ability_thdots_hinaEx_talent6:GetModifierHealthBonus()
	...
	return FindTelentValue(caster,"special_bonus_unique_hina_6")
end
...
function hina01_CreateDoll(caster , target , self)
	...
	if FindTelentValue(caster,"special_bonus_unique_hina_3") ~= 0 then
		Doll:SetPhysicalArmorBaseValue(caster:FindAbilityByName("special_bonus_unique_hina_3"):GetSpecialValueFor("armor"))
...
function modifier_ability_thdots_hina04:OnDestroy()
	...
	self.dummy = CreateUnitByName("npc_dummy_unit", 
									...
									local ability_dummy_unit = self.dummy:FindAbilityByName("ability_dummy_unit")
