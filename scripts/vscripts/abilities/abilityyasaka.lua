...
LinkLuaModifier("modifier_ability_thdots_yasakaEx_passive", "scripts/vscripts/abilities/abilityyasaka.lua",
    LUA_MODIFIER_MOTION_NONE)
function modifier_ability_thdots_yasakaEx_passive:IsHidden()
    return false
end
function modifier_ability_thdots_yasakaEx_passive:IsPurgable()
    return false
end
function modifier_ability_thdots_yasakaEx_passive:RemoveOnDeath()
    return false
end
function modifier_ability_thdots_yasakaEx_passive:IsDebuff()
    return false
end
...
function modifier_ability_thdots_yasakaEx_passive:GetAuraSearchTeam()
    return DOTA_UNIT_TARGET_TEAM_FRIENDLY
end
function modifier_ability_thdots_yasakaEx_passive:GetAuraSearchType()
    return DOTA_UNIT_TARGET_HERO
end
function modifier_ability_thdots_yasakaEx_passive:GetAuraSearchFlags()
    return DOTA_UNIT_TARGET_FLAG_NONE
end
function modifier_ability_thdots_yasakaEx_passive:GetModifierAura()
    return "modifier_ability_thdots_yasakaEx_passive_aura"
end
function modifier_ability_thdots_yasakaEx_passive:IsAura()
    return true
end
...
LinkLuaModifier("modifier_ability_thdots_yasakaEx_passive_buff", "scripts/vscripts/abilities/abilityyasaka.lua",
    LUA_MODIFIER_MOTION_NONE)

function modifier_ability_thdots_yasakaEx_passive_buff:IsPurgable()
    return false
end
function modifier_ability_thdots_yasakaEx_passive_buff:RemoveOnDeath()
    return true
end
function modifier_ability_thdots_yasakaEx_passive_buff:IsDebuff()
    return false
end
function modifier_ability_thdots_yasakaEx_passive:IsHidden()
    return false
...
function modifier_ability_thdots_yasakaEx_passive_buff:OnIntervalThink()
    ...
    if not self.target:HasModifier("modifier_ability_thdots_yasakaEx_passive_aura") then
        ...
        if self.aura:GetStackCount() > 0 then
            return
        end
    end
...
function modifier_ability_thdots_yasakaEx_passive_buff:OnAttack(keys)
    ...
    if keys.attacker ~= self.target then
        return
    end
...
LinkLuaModifier("modifier_ability_thdots_yasakaEx_passive_aura", "scripts/vscripts/abilities/abilityyasaka.lua",
    LUA_MODIFIER_MOTION_NONE)
function modifier_ability_thdots_yasakaEx_passive_aura:IsHidden()
    return false
end
function modifier_ability_thdots_yasakaEx_passive_aura:IsPurgable()
    return false
end
function modifier_ability_thdots_yasakaEx_passive_aura:RemoveOnDeath()
    return true
end
function modifier_ability_thdots_yasakaEx_passive_aura:IsDebuff()
    return false
end
function modifier_ability_thdots_yasakaEx_passive_aura:DeclareFunctions()
    return {MODIFIER_EVENT_ON_ATTACK, MODIFIER_PROPERTY_HEALTH_REGEN_PERCENTAGE,
...
function modifier_ability_thdots_yasakaEx_passive_aura:OnAttack(keys)
    ...
    if not self.target:HasModifier("modifier_ability_thdots_yasakaEx_passive_buff") then
        local buff = self.target:AddNewModifier(self.caster, self:GetAbility(),
            "modifier_ability_thdots_yasakaEx_passive_buff", {})
...
function Yasaka01_OnSpellStart(keys)
    ...
    Caster:SetContextThink("yasaka01_main_loop", function()
        if GameRules:IsGamePaused() then
            return 0.03
        end
        local enemies = FindUnitsInRadius(Caster:GetTeamNumber(), TargetPoint, nil, keys.DamageRadius,
    ...
    local effectIndex = ParticleManager:CreateParticle("particles/heroes/kanako/ability_kanako_041.vpcf",
    ...
    effectIndex = ParticleManager:CreateParticle("particles/heroes/misc/warning_circle_common.vpcf",
        PATTACH_CUSTOMORIGIN, nil)
    ParticleManager:SetParticleControl(effectIndex, 0, TargetPoint)
...
function Yasaka02_OnSpellStart(keys)
    ...
    local effectIndex = ParticleManager:CreateParticle("particles/heroes/kanako/ability_kanako_02.vpcf",
        PATTACH_CUSTOMORIGIN, nil)
    ...
    local inside_enemies = FindUnitsInRadius(Caster:GetTeamNumber(), Caster:GetOrigin(), nil, radius,
    ...
    Caster:SetContextThink("yasaka02_main_loop", function()
        if GameRules:IsGamePaused() then
            return 0.03
        end
        ...
        for k, v in pairs(inside_enemies) do
            if not v:HasModifier(keys.debuff_name) or v:IsNull() or not v:IsAlive() then
                ...
                if not v:IsPositionInRange(origin, radius) and
                    (v:HasModifier("modifier_thdots_yugi04_think_interval") == false) and v:IsHero() and
                    v:IsPositionInRange(origin, 1000) then
                    ...
                    local effectIndex1 = ParticleManager:CreateParticle(
                        "particles/econ/events/fall_major_2015/teleport_end_fallmjr_2015_l.vpcf", PATTACH_CUSTOMORIGIN,
                        nil)
        ...
        for _, v in pairs(enemies) do
            ...
            if not v:HasModifier(keys.debuff_name) and (v:HasModifier("modifier_thdots_yugi04_think_interval") == false) and
                v:IsHero() then
                local new_pos = origin + (origin - v:GetOrigin()):Normalized() * (radius + offset)
                FindClearSpaceForUnit(v, new_pos, true)
                local effectIndex2 = ParticleManager:CreateParticle(
                    "particles/econ/events/fall_major_2015/teleport_end_fallmjr_2015_l.vpcf", PATTACH_CUSTOMORIGIN, nil)
...
function ability_thdots_yasaka04_release:OnSpellStart()
    ...
    self.buff_name = Caster:AddNewModifier(Caster, self, "modifier_thdots_yasaka04_buff", {
...
LinkLuaModifier("modifier_thdots_yasaka04_buff", "scripts/vscripts/abilities/abilityyasaka.lua",
    LUA_MODIFIER_MOTION_NONE)
function modifier_thdots_yasaka04_buff:IsHidden()
    return false
end
function modifier_thdots_yasaka04_buff:IsPurgable()
    return false
end
function modifier_thdots_yasaka04_buff:RemoveOnDeath()
    return true
end
function modifier_thdots_yasaka04_buff:IsDebuff()
    return false
end
function modifier_thdots_yasaka04_buff:GetEffectName()
    return "particles/heroes/kanako/ability_kanako_04.vpcf"
end
function modifier_thdots_yasaka04_buff:GetEffectAttachType()
...
function modifier_thdots_yasaka04_buff:DeclareFunctions()
    return {MODIFIER_EVENT_ON_ATTACKED, MODIFIER_PROPERTY_ATTACK_RANGE_BONUS,
...
function modifier_thdots_yasaka04_buff:OnAttacked(keys)
    ...
    local target = keys.target
    if keys.attacker ~= self.caster or target:IsHero() == false then
        return
    end
    local damage = keys.damage * self.Damage_percent
    local TargetPoint = target:GetOrigin()
    local enemies = FindUnitsInRadius(self.caster:GetTeamNumber(), TargetPoint, nil, self.radius,
    ...
    for _, v in pairs(enemies) do
        ...
        UnitDamageTarget(damage_table)
        v:AddNewModifier(self.caster, self.Ability, "modifier_thdots_yasaka04_debuff", {
            duration = self.time * (1 - v:GetStatusResistance())
        })
    end
...
LinkLuaModifier("modifier_thdots_yasaka04_debuff", "scripts/vscripts/abilities/abilityyasaka.lua",
    LUA_MODIFIER_MOTION_NONE)
function modifier_thdots_yasaka04_debuff:IsHidden()
    return false
end
function modifier_thdots_yasaka04_debuff:IsPurgable()
    return true
end
function modifier_thdots_yasaka04_debuff:RemoveOnDeath()
    return true
end
function modifier_thdots_yasaka04_debuff:IsDebuff()
    return true
end
...
function Yasaka41_OnSpellStart(keys)
    ...
    Caster:SetContextThink("yasaka41_main_loop", function()
        if GameRules:IsGamePaused() then
            return 0.03
        end
        ...
        if last_down_tick ~= math.floor(tick * rate_tick) then
            ...
            local enemies = FindUnitsInRadius(Caster:GetTeamNumber(), rdpos, nil, keys.damage_radius,
                Ability:GetAbilityTargetTeam(), Ability:GetAbilityTargetType(), Ability:GetAbilityTargetFlags(),
                FIND_ANY_ORDER, false)
            for _, v in pairs(enemies) do
                damage_table.victim = v
...
function Yasaka42_OnSpellStart(keys)
    ...
    local effectIndex = ParticleManager:CreateParticle("particles/heroes/kanako/ability_kanako_042.vpcf",
        PATTACH_CUSTOMORIGIN, nil)
    ParticleManager:SetParticleControl(effectIndex, 0, TargetPoint)
    ParticleManager:SetParticleControl(effectIndex, 7, TargetPoint)
    ParticleManager:DestroyParticleSystemTime(effectIndex, keys.duration)

    Caster:SetContextThink("yasaka42_main_loop", function()
        if GameRules:IsGamePaused() then
            return 0.03
        end
        if Ability:IsNull() then
        ...
        local enemies = FindUnitsInRadius(Caster:GetTeamNumber(), TargetPoint, nil, keys.effect_radius,
            Ability:GetAbilityTargetTeam(), Ability:GetAbilityTargetType(), Ability:GetAbilityTargetFlags(),
            FIND_ANY_ORDER, false)
        for _, v in pairs(enemies) do
            Ability:ApplyDataDrivenModifier(Caster, v, keys.debuff_name, {})
...
function Yasaka43_OnSpellStart(keys)
    ...
    local origin = Caster:GetOrigin()
    local effectIndex_2 = ParticleManager:CreateParticle(
        "particles/econ/items/legion/legion_weapon_voth_domosh/legion_duel_start_ring_flash_arcana.vpcf",
        PATTACH_CUSTOMORIGIN, nil)
    ...
    local effectIndex = ParticleManager:CreateParticle(
        "particles/units/heroes/hero_treant/treant_overgrowth_vine_glow_trail.vpcf", PATTACH_CUSTOMORIGIN, nil)
    ...
    Caster:SetContextThink("yasaka43_main_loop", function()
        if GameRules:IsGamePaused() then
            return 0.03
        end
        if Ability:IsNull() then
