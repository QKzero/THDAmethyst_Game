...
function MargatroidEx_ModifyDollActionTranslate(Caster)
    ...
    Caster:SetContextThink("MargatroidEx_ModifyDollActionTranslate", function()
...
function Margatroid_CreateDoll(hCaster, vecPos, vecForward, dollname)
    ...
    if AbilityEx then
        local doll = CreateUnitByName(MARGATROID_ABILITYEX_DOLL_UNITNAME, vecPos, false, hCaster, hCaster,
            hCaster:GetTeam())
        SetTHD2BlockingNeutrals(doll, false)
...
function Margatroid_FindDolls(hCaster, fRadius)
    local units = FindUnitsInRadius(hCaster:GetTeamNumber(), hCaster:GetOrigin(), nil, fRadius,
        DOTA_UNIT_TARGET_TEAM_FRIENDLY, DOTA_UNIT_TARGET_BASIC,
        DOTA_UNIT_TARGET_FLAG_INVULNERABLE + DOTA_UNIT_TARGET_FLAG_NOT_DOMINATED, FIND_ANY_ORDER, false)
    local dolls = {}
    for _, v in pairs(units) do
        if v:HasModifier(MARGATROID_ABILITYEX_MODIFIER_DOLL_NAME) and
            not v:HasModifier(MARGATROID_ABILITYEX_MODIFIER_DOLL_RECOVERING_NAME) then
...
function Margatroid_MoveDoll(hDoll, vecTarget, fMoveSpeed, fnOnMoving, fnOnFinshMove, isfly)
    ...
    hDoll:SetContextThink("margatroid_move_doll_" .. tostring(vecTarget), function()
        ...
        if fnOnMoving and fnOnMoving(hDoll) then
            finish_move = true
        end
        if finish_move then
...
function MargatroidEx_AddDoll(keys)
    local AbilityEx = keys.ability
    local Caster = keys.caster
    Caster:SetContextThink(DoUniqueString("margatroid_ex_doll_recovering_reset"), function()
...
function MargatroidEx_OnDollDestroy(keys)
    ...
    local Caster = keys.caster
    local doll = keys.target
    if doll:HasModifier(MARGATROID_ABILITYEX_MODIFIER_DOLL_RECOVERING_NAME) then
        return
    end
...
function MargatroidEx_OnDollRecovering(keys)
    ...
    if doll:HasModifier(MARGATROID_ABILITYEX_MODIFIER_DOLL_NAME) and
        doll:HasModifier(MARGATROID_ABILITYEX_MODIFIER_DOLL_RECOVERING_NAME) and not Caster:IsStunned() then
        ...
        local vecMovePos = vecDollPos + (vecCasterPos - vecDollPos):Normalized() * keys.ThinkInterval *
                               keys.RecoveringMoveSpeed
        if (vecCasterPos - vecMovePos):Length2D() < 50 then

            AbilityEx:ApplyDataDrivenModifier(Caster, Caster, MARGATROID_ABILITYEX_MODIFIER_RESET_DOLL_NAME, {
                Duration = keys.ResetDollTime
            })

            local effectIndex = ParticleManager:CreateParticle("particles/heroes/alice/ability_alice_ex_release.vpcf",
                PATTACH_CUSTOMORIGIN, nil)
            ParticleManager:SetParticleControlEnt(effectIndex, 0, Caster, 5, "attach_line", Vector(0, 0, 0), true)
...
function Margatroid01_OnSpellStart(keys)
    ...
    for _, tab in pairs(dolls_and_pos) do
        local OnDollFinshMove = function(hDoll)
            hDoll:StartGesture(ACT_DOTA_ATTACK)
            hDoll:SetForwardVector(direction)
            local effectIndex = ParticleManager:CreateParticle("particles/heroes/alice/ability_alice_dash_a1.vpcf",
                PATTACH_CUSTOMORIGIN, hDoll)
            ...
            hDoll:SetContextThink("margatroid01_action_delay", function()
                Ability:ApplyDataDrivenModifier(Caster, hDoll, keys.ModifierGoForward, {
                    Duration = goforward_duration
                })
            end, 0.1)
        end
        if tab.doll then
            Margatroid_MoveDoll(tab.doll, tab.start_pos, keys.Radius * 2, nil, OnDollFinshMove, true)
        end
    end
end
...
function Margatroid01_IfKnockback(keys)
    ...
    if not Target:HasModifier(keys.ModifierKnockbackName) then
        Ability:ApplyDataDrivenModifier(Caster, Target, keys.ModifierKnockbackName, {
            Duration = keys.KnockbackDuration
        })
        Margatroid01_KnockbackTarget(Caster, Target, Caster:GetForwardVector(), keys.KnockbackDistance,
            keys.KnockbackDuration, keys.ModifierKnockbackName)
        UnitDamageTarget {
            ability = Ability,
            victim = Target,
            attacker = Caster,
...
function Margatroid01_KnockbackTarget(hCaster, hTarget, vecDirection, fDistance, fDuration, strModifierKnockbackName)
    ...
    hTarget:SetContextThink("Margatroid01_Knockback", function()
        tick = tick + 1
...
function Margatroid02_OnSpellStart(keys)
    ...
    elseif usable_doll_num == 1 then
        if focus_target then
            ...
            local doll = Margatroid_CreateDoll(Caster, caster_pos + (start_pos - caster_pos):Normalized() * 20,
                doll_dir, "FALANXI")
            dolls_and_pos[1] = {
                doll = doll,
                start_pos = start_pos
    ...
    for _, tab in pairs(dolls_and_pos) do
        local OnDollFinshMove = function(hDoll)
            ...
            hDoll:SetContextThink("margatroid02_action_delay", function()
                local effectIndex = ParticleManager:CreateParticle("particles/heroes/alice/ability_alice_02.vpcf",
                    PATTACH_CUSTOMORIGIN, hDoll)

                ParticleManager:SetParticleControl(effectIndex, 0, hDoll:GetAttachmentOrigin(
                    hDoll:ScriptLookupAttachment("attach_attack1")) + Vector(0, 0, 30))
                ParticleManager:SetParticleControl(effectIndex, 1, hDoll:GetOrigin() + hDoll:GetForwardVector() * 600)
                ParticleManager:SetParticleControl(effectIndex, 9, hDoll:GetAttachmentOrigin(
                    hDoll:ScriptLookupAttachment("attach_attack1")) + Vector(0, 0, 30))
                ...
                local enemies = FindUnitsInRadius(Caster:GetTeamNumber(), doll_pos + target_dir * keys.Distance * 0.5,
                    nil, keys.Distance, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC,
        ...
        if tab.doll then
            Margatroid_MoveDoll(tab.doll, tab.start_pos, keys.Radius * 2, nil, OnDollFinshMove, false)
        end
    end
end

modifier_thdots_margatroid03_doll_Upgrade = {}
LinkLuaModifier("modifier_thdots_margatroid03_doll_Upgrade", "scripts/vscripts/abilities/abilitymargatroid.lua",
    LUA_MODIFIER_MOTION_NONE)
function modifier_thdots_margatroid03_doll_Upgrade:IsHidden()
    return true
end
function modifier_thdots_margatroid03_doll_Upgrade:IsPurgable()
    return false
end
function modifier_thdots_margatroid03_doll_Upgrade:RemoveOnDeath()
    return true
end
function modifier_thdots_margatroid03_doll_Upgrade:IsDebuff()
    return false
end

function modifier_thdots_margatroid03_doll_Upgrade:DeclareFunctions()
    local funcs = {MODIFIER_PROPERTY_ATTACKSPEED_BONUS_CONSTANT, MODIFIER_PROPERTY_PHYSICAL_ARMOR_BONUS}
    return funcs
end

function modifier_thdots_margatroid03_doll_Upgrade:AddCustomTransmitterData()
    return {
        attackSpeed = self.attackSpeed,
        armor = self.armor
    }
end
function modifier_thdots_margatroid03_doll_Upgrade:HandleCustomTransmitterData(data)
    self.attackSpeed = data.attackSpeed
    self.armor = data.armor
end

function modifier_thdots_margatroid03_doll_Upgrade:OnCreated()
    if not IsServer() then
        return
    end
    self.caster = self:GetCaster();
    self.attackSpeed = self.caster:GetDisplayAttackSpeed() *
                           FindTelentValue(self.caster, "special_bonus_unique_margatroid_5") / 100
    self.armor = self.caster:GetPhysicalArmorValue(false) *
                     FindTelentValue(self.caster, "special_bonus_unique_margatroid_5") / 100
    self:SetHasCustomTransmitterData(true)
    self:StartIntervalThink(FrameTime())
end
function modifier_thdots_margatroid03_doll_Upgrade:OnIntervalThink()
    if not IsServer() then
        return
    end
    self.attackSpeed = self.caster:GetDisplayAttackSpeed() *
...
function modifier_thdots_margatroid03_doll_Upgrade:GetModifierAttackSpeedBonus_Constant()
    if self.attackSpeed == nil then
        self.attackSpeed = 0
    end
    return self.attackSpeed
end
function modifier_thdots_margatroid03_doll_Upgrade:GetModifierPhysicalArmorBonus()
...
function Margatroid03_OnSpellStart(keys)
    ...
    local doll = CreateUnitByName(MARGATROID_ABILITY03_DOLL_UNITNAME,
        Caster:GetOrigin() + Caster:GetForwardVector() * 100, true, Caster, Caster, Caster:GetTeamNumber())
    ...
    doll:SetContextThink(DoUniqueString("margatroid_03_dash"), function()
        if GameRules:IsGamePaused() then
            return 0.03
        end
        local speed = 2800 * 0.03
        distance = speed + distance
        local enemies = FindUnitsInRadius(Caster:GetTeam(), doll:GetOrigin(), nil, 100, DOTA_UNIT_TARGET_TEAM_ENEMY,
...
function Margatroid03_DollOnIntervalThink(keys)
    ...
    if FindTelentValue(Caster, "special_bonus_unique_margatroid_0") == 0 then
        ...
        elseif distance > keys.DollMaxDistance then
            local new_pos = Caster:GetOrigin() + (Doll:GetOrigin() - Caster:GetOrigin()):Normalized() *
                                keys.DollMaxDistance
...
function Margatroid33_OnSpellStart(keys)
    ...
    ParticleManager:SetParticleControl(effectIndex, 0, Caster:GetAttachmentOrigin(
        Caster:ScriptLookupAttachment("attach_cast")) + Vector(0, 0, 160))
    ParticleManager:SetParticleControl(effectIndex, 1,
        Caster:GetAttachmentOrigin(Caster:ScriptLookupAttachment("attach_cast")) + Caster:GetForwardVector() * 600 +
            Vector(0, 0, 160))
    ParticleManager:SetParticleControl(effectIndex, 9, Caster:GetAttachmentOrigin(
        Caster:ScriptLookupAttachment("attach_cast")) + Vector(0, 0, 160))
    ...
    local enemies = FindUnitsInRadius(Caster:GetTeamNumber(), caster_pos + target_dir * keys.Distance * 0.5, nil,
...
function Margatroid04_OnSpellStart(keys)
    ...
    Caster:EmitSound("visage_visa_attack_04")
    for _, doll in pairs(dolls) do
        ...
        doll:SetContextThink("Margatroid04_action", function()
            ExecuteOrderFromTable {
                UnitIndex = doll:entindex(),
...
function ability_thdots_Margatroid_lyz:GetAOERadius()
    return self:GetSpecialValueFor("damage_radius")
end

function ability_thdots_Margatroid_lyz:OnSpellStart()
    local Ability = self
    ...
    self.caster_direction = Caster:GetForwardVector() * Vector(-1, -1, 0) -- 面向的反方向

    local radius = self:GetSpecialValueFor("radius")
    local mana_cost = Ability:GetManaCost(Ability:GetLevel())
    local move_speed = self:GetSpecialValueFor("doll_move_speed")
    local move_distance = self:GetSpecialValueFor("doll_move_distance")
    ...
    if usable_doll_num == 0 or not targetPoint then
        Ability:EndCooldown()
        Ability:RefundManaCost()
        return
    elseif usable_doll_num == 1 then
        local start_pos = caster_pos - direction * radius
        local doll = Margatroid_CreateDoll(Caster, caster_pos + (start_pos - caster_pos):Normalized() * 20, direction,
            "FALANXI")
        dolls_and_pos[1] = {
            doll = doll,
    ...
    elseif usable_doll_num == 2 then
        Caster:SpendMana(mana_cost * (usable_doll_num - 1), Ability)
        local base_pos = caster_pos - direction * radius
    ...
    -- local goforward_duration=(move_distance+radius)/move_speed
    local move_duration = self:GetSpecialValueFor("caster_move_duration")
    Caster:AddNewModifier(Caster, self, "modifier_thdots_margatroid_lyz_back_moving", {
        duration = move_duration
    })

    for _, tab in pairs(dolls_and_pos) do
        local OnDollFinshMove = function(hDoll)
            local vecDoll = hDoll:GetOrigin()
            local rad = GetRadBetweenTwoVec2D(vecDoll, targetPoint)
            local distance = GetDistanceBetweenTwoVec2D(vecDoll, targetPoint)
            if distance > move_distance then
                distance = move_distance
            end
            local high = vecDoll.z
            local fallspeed = high / (distance / 35)
            local distance_end = distance
            local damage = self:GetSpecialValueFor("damage_per_doll")
            ...
            --[[hDoll:SetContextThink(
					...
					function ()
						Ability:ApplyDataDrivenModifier(Caster,hDoll,"modifier_thdots_margatroid01_doll_go_forward",{Duration=goforward_duration})
					end,0.1)--]]

            hDoll:SetContextThink(DoUniqueString("ability_thdots_Margatroid_lyz_doll_destory"), -- 幽灵船自爆
            function()
                if GameRules:IsGamePaused() then
                    return 0.03
                end
                distance_end = distance_end - move_speed
                if distance_end >= 0 then
                    hDoll:SetOrigin(hDoll:GetOrigin() +
                                        Vector(math.cos(rad) * move_speed, math.sin(rad) * move_speed, -fallspeed))
                    return 0.04
                else
                    local targets = FindUnitsInRadius(Caster:GetTeam(), hDoll:GetOrigin(), nil, damage_radius,
                        self:GetAbilityTargetTeam(), self:GetAbilityTargetType(), 0, FIND_CLOSEST, false)
                    for k, v in pairs(targets) do
                        local damage_table = {
                            ability = self,
                            victim = v,
        ...
        if tab.doll then
            Margatroid_MoveDoll(tab.doll, tab.start_pos, radius * 4, nil, OnDollFinshMove, true)
        end
    end
end

modifier_thdots_margatroid_lyz_back_moving = {}
LinkLuaModifier("modifier_thdots_margatroid_lyz_back_moving", "scripts/vscripts/abilities/abilityMargatroid.lua",
    LUA_MODIFIER_MOTION_NONE)

function modifier_thdots_margatroid_lyz_back_moving:IsHidden()
    return false
end
function modifier_thdots_margatroid_lyz_back_moving:IsPurgable()
    return false
end
function modifier_thdots_margatroid_lyz_back_moving:RemoveOnDeath()
    return true
end
function modifier_thdots_margatroid_lyz_back_moving:IsDebuff()
    return false
end

function modifier_thdots_margatroid_lyz_back_moving:GetEffectAttachType()
    return PATTACH_ABSORIGIN
end

function modifier_thdots_margatroid_lyz_back_moving:GetAttributes()
    return MODIFIER_ATTRIBUTE_IGNORE_INVULNERABLE
end
function modifier_thdots_margatroid_lyz_back_moving:DeclareFunctions()
    return {MODIFIER_PROPERTY_INVISIBILITY_LEVEL}
end

function modifier_thdots_margatroid_lyz_back_moving:GetOverrideAnimation()
    return ACT_DOTA_CAST_ABILITY_1
end

function modifier_thdots_margatroid_lyz_back_moving:CheckState()
    return {
...
function modifier_thdots_margatroid_lyz_back_moving:GetModifierInvisibilityLevel() -- 模型隐身效果，需要MODIFIER_PROPERTY_INVISIBILITY_LEVEL
    return 1
end

function modifier_thdots_margatroid_lyz_back_moving:OnCreated()
    if not IsServer() then
        return
    end
    local effectIndex = ParticleManager:CreateParticle(
        "particles/units/heroes/hero_bane/bane_fiendsgrip_ground_rubble.vpcf", PATTACH_WORLDORIGIN, nil)
...
function modifier_thdots_margatroid_lyz_back_moving:OnDestroy()
    local effectIndex = ParticleManager:CreateParticle(
        "particles/econ/items/bane/bane_fall20_immortal/bane_fall20_immortal_grip_ring.vpcf", PATTACH_WORLDORIGIN, nil)
    ParticleManager:SetParticleControl(effectIndex, 1, self:GetParent():GetAbsOrigin())
    ParticleManager:ReleaseParticleIndex(effectIndex)
end

function modifier_thdots_margatroid_lyz_back_moving:OnIntervalThink()
    local caster = self:GetCaster()
