...
function MargatroidEx_IntervalAddDoll(keys)
	...
	local Caster=keys.caster
	local MaxStoreNum=keys.DollBaseNum+math.floor((Caster:GetLevel()-1)/keys.LvlToUpgrade) + FindTelentValue(Caster,"special_bonus_unique_margatroid_2")--人偶上限在这里
...
LinkLuaModifier("modifier_thdots_margatroid03_doll_Upgrade","scripts/vscripts/abilities/abilitymargatroid.lua",LUA_MODIFIER_MOTION_NONE)
function modifier_thdots_margatroid03_doll_Upgrade:IsHidden()       return true end
function modifier_thdots_margatroid03_doll_Upgrade:IsPurgable()     return false end
function modifier_thdots_margatroid03_doll_Upgrade:RemoveOnDeath()  return true end
function modifier_thdots_margatroid03_doll_Upgrade:IsDebuff()       return false end
...
function modifier_thdots_margatroid03_doll_Upgrade:OnCreated()
	...
	self.attackSpeed = self.caster:GetDisplayAttackSpeed()*FindTelentValue(self.caster,"special_bonus_unique_margatroid_5")/100
	self.armor = self.caster:GetPhysicalArmorValue(false)*FindTelentValue(self.caster,"special_bonus_unique_margatroid_5")/100
...
function ability_thdots_Margatroid_lyz:OnSpellStart()
	...
	Caster:AddNewModifier(Caster, self, "modifier_thdots_margatroid_lyz_back_moving", {duration = move_duration})
	...
	for _,tab in pairs(dolls_and_pos) do
		local OnDollFinshMove=
			function (hDoll)
				...
				if distance > move_distance then distance = move_distance end
				...
				--[[hDoll:SetContextThink(
					...
					function ()
						...
						if distance_end >= 0 then
							hDoll:SetOrigin(hDoll:GetOrigin() + Vector(math.cos(rad)*move_speed,math.sin(rad)*move_speed,-fallspeed))
							return 0.04
						else
...
LinkLuaModifier("modifier_thdots_margatroid_lyz_back_moving", "scripts/vscripts/abilities/abilityMargatroid.lua", LUA_MODIFIER_MOTION_NONE)

function modifier_thdots_margatroid_lyz_back_moving:IsHidden() return false end
function modifier_thdots_margatroid_lyz_back_moving:IsPurgable() return false end
function modifier_thdots_margatroid_lyz_back_moving:RemoveOnDeath() return true end
function modifier_thdots_margatroid_lyz_back_moving:IsDebuff() return false end
...
function modifier_thdots_margatroid_lyz_back_moving:OnCreated()
	if not IsServer() then return end
	local effectIndex = ParticleManager:CreateParticle("particles/units/heroes/hero_bane/bane_fiendsgrip_ground_rubble.vpcf", PATTACH_WORLDORIGIN, nil)
...
function modifier_thdots_margatroid_lyz_back_moving:OnDestroy()
	local effectIndex = ParticleManager:CreateParticle("particles/econ/items/bane/bane_fall20_immortal/bane_fall20_immortal_grip_ring.vpcf", PATTACH_WORLDORIGIN, nil)
