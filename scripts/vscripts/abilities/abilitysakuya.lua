...
function OnSakuyaExSpellStart(keys)
    ...
    if (caster.ability_sakuya_01_stun == FALSE or caster.ability_sakuya_01_stun == nil) then
        ...
        local effectIndex = ParticleManager:CreateParticle("particles/heroes/sakuya/ability_sakuya_ex.vpcf",
...
function OnSakuya01SpellStart(keys)
    ...
    if (caster.ability_sakuya_01_stun == TRUE) then
        ...
        local effectIndex = ParticleManager:CreateParticle("particles/heroes/sakuya/ability_sakuya_ex_stun.vpcf",
            PATTACH_CUSTOMORIGIN, caster)
    ...
    if (caster.ability_sakuya_ex_index ~= -1 and caster.ability_sakuya_ex_index ~= nil) then
        ...
        if caster:HasModifier("modifier_thdots_sakuyaEx_flag") then
            caster:RemoveModifierByName("modifier_thdots_sakuyaEx_flag")
        end
    end
    ...
    Timer.Wait 'ability_sakuya_01_stun_timer'(0.5, function()
...
function OnSakuya02SpellDamage(keys)
    ...
    local targetPoint = Vector(keys.ability.ability_sakuya02_point_x, keys.ability.ability_sakuya02_point_y,
        keys.ability.ability_sakuya02_point_z)
    ...
    for i = 1, keys.ability:GetLevel() do
        local iVec = Vector(math.cos(pointRad + math.pi / 18 * (i + 0.5)) * 2000,
            math.sin(pointRad + math.pi / 18 * (i + 0.5)) * 2000, 0)
        knifeTable.vVelocity = iVec
        ProjectileManager:CreateLinearProjectile(knifeTable)
        iVec = Vector(math.cos(pointRad - math.pi / 18 * (i + 0.5)) * 2000,
            math.sin(pointRad - math.pi / 18 * (i + 0.5)) * 2000, 0)
        knifeTable.vVelocity = iVec
        ProjectileManager:CreateLinearProjectile(knifeTable)
    end
    ...
    for _, v in pairs(targets) do
        ...
        if IsPointInCircularSector(vVec.x, vVec.y, math.cos(vecRad), math.sin(vecRad), keys.DamageRadius,
            math.pi * (keys.DamageRad / 180), vecCaster.x, vecCaster.y) then
            -- local intBouns = (caster:GetIntellect(false)	- (caster:GetIntellect(false)%6)) / 6 * keys.IntMulti + 1
            -- local agiBouns = (caster:GetAgility() - (caster:GetAgility()%6)) / 6 * keys.AgiMulti
            -- print("----------------------")
            -- print(intBouns)
...
function ability_thdots_sakuya03:OnSpellStart()
    ...
    local targets = FindUnitsInRadius(caster:GetTeam(), caster:GetAbsOrigin(), nil,
        self:GetSpecialValueFor("damage_radius"), DOTA_UNIT_TARGET_TEAM_ENEMY,
    ...
    local effectIndex = ParticleManager:CreateParticle("particles/thd2/heroes/sakuya/ability_sakuya_03.vpcf",
        PATTACH_CUSTOMORIGIN, caster)
    ...
    caster:AddNewModifier(caster, self, "modifier_thdots_sakuya03_bouns_attackspeed", {
        duration = duration
    })
...
LinkLuaModifier("modifier_thdots_sakuya03_bouns_attackspeed", "scripts/vscripts/abilities/abilitysakuya.lua",
    LUA_MODIFIER_MOTION_NONE)
function modifier_thdots_sakuya03_bouns_attackspeed:IsHidden()
    return false
end
function modifier_thdots_sakuya03_bouns_attackspeed:IsDebuff()
    return false
end
function modifier_thdots_sakuya03_bouns_attackspeed:IsPurgable()
    return true
end
function modifier_thdots_sakuya03_bouns_attackspeed:RemoveOnDeath()
    return true
end
function modifier_thdots_sakuya03_bouns_attackspeed:GetAttributes()
...
function modifier_thdots_sakuya03_bouns_attackspeed:GetModifierAttackSpeedBonus_Constant()
...
function OnSakuya04SpellStart(keys)
    ...
    local nEffectIndex = ParticleManager:CreateParticle("particles/thd2/heroes/sakuya/ability_sakuya_04.vpcf",
    ...
    if caster:HasModifier("modifier_item_wanbaochui") then
        keys.ability:ApplyDataDrivenModifier(caster, unit, "modifier_the_world", {
            duration = 2
        })
    end
    ...
    GameRules:GetGameModeEntity():SetContextThink(DoUniqueString('ability_sakuya04_remove'), function()
