function OnUtsuho01SpellStart(keys)
    ...
    if FindTelentValue(caster, "special_bonus_unique_utsuho_2") ~= 0 then
        local targets = FindUnitsInRadius(caster:GetTeam(), targetLoc, nil, ability:GetSpecialValueFor("radius"),
        ...
        local effectIndex = ParticleManager:CreateParticle("particles/thd2/heroes/utsuho/ability_utsuho01_effect.vpcf",
...
function OnUtsuho03SpellEnd(keys)
    ...
    print("args")
    local utsuho03_particle = ParticleManager:CreateParticle(
        "particles/thd2/heroes/utsuho/ability_utsuho03_effect.vpcf", PATTACH_OVERHEAD_FOLLOW, caster)
...
function OnUtsuho04SpellStart(keys)
    ...
    if caster:HasModifier("modifier_item_wanbaochui") then
        local targets = FindUnitsInRadius(caster:GetTeam(), targetPoint, nil, 750, DOTA_UNIT_TARGET_TEAM_ENEMY,
    ...
    local dummy = CreateUnitByName("npc_dummy_unit", targetPoint, false, caster, caster, caster:GetTeamNumber())
    ...
    dummy:SetContextThink("ability_utsuho04_effect_release", function()
        if GameRules:IsGamePaused() then
            return 0.03
        end
        dummy:RemoveSelf()
    end, 7.5)
    local effectIndex = ParticleManager:CreateParticle("particles/thd2/heroes/utsuho/ability_utsuho04_effect.vpcf",
        PATTACH_CUSTOMORIGIN, dummy)
...
function OnUtsuho04SpellRemove(keys)
    ...
    if (caster.ability_utsuho_04_dummy ~= nil) then
        effectIndex = ParticleManager:CreateParticle("particles/thd2/heroes/utsuho/ability_utsuho04_end.vpcf",
            PATTACH_CUSTOMORIGIN, caster.ability_utsuho_04_dummy)
    else
        effectIndex = ParticleManager:CreateParticle("particles/thd2/heroes/utsuho/ability_utsuho04_end.vpcf",
    ...
    caster.ability_utsuho_04_dummy:SetContextThink("ability_utsuho04_effect_remove", function()
        if GameRules:IsGamePaused() then
            return 0.03
        end
