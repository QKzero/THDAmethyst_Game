function OnStar05SpellStart(keys)
    ...
    ability:ApplyDataDrivenModifier(caster, caster, "modifier_thdotsr_star05_think_interval", {
        duration = spell_duration
    })

    ability:ApplyDataDrivenModifier(caster, caster, "modifier_thdotsr_star05_think_interval_effect", {
        duration = spell_duration
    })
end
...
function OnStar05SpellThink(keys)
    ...
    DOTA_UNIT_TARGET_TEAM_ENEMY, target_type, 0, -- flag
    ...
    local deal_damage = ((base_damage + caster:GetIntellect(false) * int_scale) * intervals) / spell_duration

    for _, v in pairs(targets) do
        if FindTelentValue(caster, "special_bonus_unique_star_4") ~= 0 then
            local heroes = FindUnitsInRadius(caster:GetTeam(), caster:GetOrigin(), nil, search_radius,
        ...
        local effectIndex = ParticleManager:CreateParticle(
            "particles/units/heroes/hero_mirana/mirana_starfall_attack.vpcf", PATTACH_ABSORIGIN_FOLLOW, v)
        ParticleManager:SetParticleControl(effectIndex, 0, v:GetAbsOrigin())
        v:SetContextThink("Star05_Think", function()
    ...
    local healing = (heal + caster:GetIntellect(false) * heal_int_Scale) * intervals / duration
...
function OnStar05SpellThinkEffect(keys)
    ...
    for _, v in pairs(STAR_UNITS) do
        ...
        local summonVector = Vector(math.cos(rollRad) * forwardCos - math.sin(rollRad) * forwardSin,
            forwardSin * math.cos(rollRad) + forwardCos * math.sin(rollRad), 0) * summon_range + vecCaster
...
function OnStar05EffectCreated(keys)
    ...
    for i = 1, max_spirit do
        ...
        local ability_dummy_unit = newSpirit:FindAbilityByName("ability_dummy_unit")
        ...
        newSpirit:SetContextThink("dummy_kill", function()
            if GameRules:IsGamePaused() then
                return 0.03
            end
        ...
        end, FindValueTHD("duration", ability))
...
function ability_thdotsr_star01:OnSpellStart()
    ...
    local effectIndex = ParticleManager:CreateParticle(
        "particles/econ/items/mirana/mirana_starstorm_bow/mirana_starstorm_starfall_attack.vpcf", PATTACH_WORLDORIGIN,
        caster)
    ...
    local deal_damage = (base_damage + caster:GetIntellect(false) * int_scale)

    local stun_duration = FindValueTHD("stun_duration", ability) +
                              FindTelentValue(caster, "special_bonus_unique_star_1")

    if FindTelentValue(caster, "special_bonus_unique_star_2") ~= 0 and
        not caster:HasModifier("modifier_ability_thdots_star_talent_2") then
    ...
    caster:SetContextThink("OnStar01SpellStart", function()
        if GameRules:IsGamePaused() then
            return 0.03
        end
        ...
        nil, -- find entity
        search_radius, -- find radius
        DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_CREEP + DOTA_UNIT_TARGET_HERO, 0, -- flag
        FIND_ANY_ORDER, false)
        for _, v in pairs(targets) do
            local damage_table = {
                ability = ability,
...
function OnStar03SpellStart(keys)
    ...
    SetTHD2BlockingNeutrals(starWard, false)
    starWard:SetBaseMaxHealth(attack_count)
    local spell_duration = FindValueTHD("duration", ability)
    ability:ApplyDataDrivenModifier(caster, starWard, "modifier_thdotsr_star03_ward", {
        duration = spell_duration
    })
    starWard:SetModifierStackCount("modifier_thdotsr_star03_ward", caster, attack_count)
    starWard:AddNewModifier(caster, ability, "modifier_item_ward_true_sight", {
        duration = spell_duration,
        true_sight_range = true_sight_range
    })
    starWard:AddNewModifier(caster, ability, "modifier_kill", {
        duration = spell_duration
    })
    ...
    starWard:SetContextThink("Star03_flash", function()
        ...
        if starWard:IsAlive() then
            local effectIndex = ParticleManager:CreateParticle(
                ...
                PATTACH_CUSTOMORIGIN_FOLLOW, starWard)
...
function ability_thdotsr_star02:OnSpellStart()
    ...
    if FindTelentValue(caster, "special_bonus_unique_star_3") ~= 0 and
        not caster:HasModifier("modifier_ability_thdots_star_talent_3") then
        caster:AddNewModifier(caster, self, "modifier_ability_thdots_star_talent_3", {})
    end
    self:GetCursorTarget():AddNewModifier(self:GetCaster(), self, "modifier_ability_thdotsr_star02_curse", {
        duration = self:GetSpecialValueFor("snare_duration") * (1 - target:GetStatusResistance())
    })
    target:EmitSound("DOTA_Item.RodOfAtos.Target")
    if FindTelentValue(caster, "special_bonus_unique_star_3") ~= 0 then
        local targets = FindUnitsInRadius(caster:GetTeam(), target:GetOrigin(), nil,
            FindTelentValue(caster, "special_bonus_unique_star_3"), self:GetAbilityTargetTeam(),
            self:GetAbilityTargetType(), 0, 0, false)
        for _, v in pairs(targets) do
            if v ~= target then
                v:AddNewModifier(self:GetCaster(), self, "modifier_ability_thdotsr_star02_curse", {
                    duration = self:GetSpecialValueFor("snare_duration") * (1 - v:GetStatusResistance())
                })
...
LinkLuaModifier("modifier_ability_thdotsr_star02_curse", "scripts/vscripts/abilities/abilitystar.lua",
...
function modifier_ability_thdotsr_star02_curse:OnAbilityExecuted(keys)
    ...
    if target:HasModifier("modifier_ability_thdotsr_star02_curse") then
        ...
        if caster:HasModifier("modifier_item_wanbaochui") then
            ...
            local targets = FindUnitsInRadius(caster:GetTeam(), target:GetOrigin(), nil, 500,
                ability:GetAbilityTargetTeam(), DOTA_UNIT_TARGET_BASIC + DOTA_UNIT_TARGET_HERO, 0, 0, false)

            local void_pfx = ParticleManager:CreateParticle(
                "particles/units/heroes/hero_antimage/antimage_manavoid.vpcf", PATTACH_POINT_FOLLOW, target)
...
LinkLuaModifier("modifier_ability_thdots_star_talent_3", "scripts/vscripts/abilities/abilitystar.lua",
    LUA_MODIFIER_MOTION_NONE)
function modifier_ability_thdots_star_talent_3:IsHidden()
    return true
end
function modifier_ability_thdots_star_talent_3:IsPurgable()
    return false
end
function modifier_ability_thdots_star_talent_3:RemoveOnDeath()
    return false
end
function modifier_ability_thdots_star_talent_3:IsDebuff()
    return false
end

modifier_ability_thdots_star_talent_2 = {}
LinkLuaModifier("modifier_ability_thdots_star_talent_2", "scripts/vscripts/abilities/abilitystar.lua",
    LUA_MODIFIER_MOTION_NONE)
function modifier_ability_thdots_star_talent_2:IsHidden()
    return true
end
function modifier_ability_thdots_star_talent_2:IsPurgable()
    return false
end
function modifier_ability_thdots_star_talent_2:RemoveOnDeath()
    return false
end
function modifier_ability_thdots_star_talent_2:IsDebuff()
    return false
end
