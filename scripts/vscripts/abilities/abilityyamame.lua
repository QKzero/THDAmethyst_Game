...
function ability_thdots_yamame04:CastFilterResultTarget(target)
	if target == self:GetCaster() then
		if IsServer() then self:ToggleAutoCast() end
...
function ability_thdots_yamame04:OnSpellStart()
	...
	self.cast_range				= self:GetEffectiveCastRange(self:GetCursorPosition(), nil)
	...
	local hookshot_duration	= (self.cast_range / self.hook_speed)
	local hookshot_particle = ParticleManager:CreateParticle("particles/thd2/heroes/yamame/yamame_04_hook.vpcf", PATTACH_CUSTOMORIGIN, nil)
	...
	ParticleManager:SetParticleControl(hookshot_particle, 1, self:GetCaster():GetAbsOrigin() + self.direction * self.cast_range)
