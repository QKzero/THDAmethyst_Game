...
function CirnoCreateIcebound(caster, pos, lifetime)
    ...
    local effect_index = ParticleManager:CreateParticle(
        "particles/units/heroes/hero_invoker/invoker_ice_wall_snow_ground.vpcf", PATTACH_CUSTOMORIGIN, unit)
...
function CirnoBreakIcebound(icebound, OnBreak, count)
    ...
    for _, ib in pairs(icebounds) do
        ...
        Timer.Loop(tostring(ib) .. "ready_to_break")(ABILITY_CIRNO_ICEBOUND_BREAK_DELAY, 1, function()
...
function CirnoBreakIceboundsInRadius(caster, pos, radius)
    ...
    if AbilityCirno02 then
        cirno02_StunEnemyDuration = AbilityCirno02:GetLevelSpecialValueFor("stun_enemy_duration",
            AbilityCirno02:GetLevel())
        cirno02_damage_table = {
            victim = nil,
            attacker = caster,
            damage = AbilityCirno02:GetSpecialValueFor("damage"),
            damage_type = AbilityCirno02:GetAbilityDamageType(),
            ability = AbilityCirno02
        }
    end
    ...
    for _, ib in pairs(icebounds) do
        if not ib.ready_to_break then
            ...
            CirnoBreakIcebound(ib, function(icebound, count, nearby_icebounds)
                ...
                local enemies = FindUnitsInRadius(caster:GetTeamNumber(), icebound.pos, nil,
                    ABILITY_CIRNO_ICEBOUND_DAMAGE_RANGE, DOTA_UNIT_TARGET_TEAM_ENEMY,
                ...
                local effectIndex = ParticleManager:CreateParticle("particles/heroes/cirno/ability_cirno_02.vpcf",
...
function OnCirno01SpellStart(keys)
    ...
    local Targets = FindUnitsInRadius(Caster:GetTeamNumber(), Caster:GetAbsOrigin(), nil, keys.radius,
        Ability:GetAbilityTargetTeam(), Ability:GetAbilityTargetType(), Ability:GetAbilityTargetFlags(), FIND_ANY_ORDER, --[[FIND_CLOSEST,]]
        false)
    local damage_table = {
        victim = nil,
        attacker = Caster,
...
function OnCirno03SpellStart(keys)
    ...
    local HasDamaged = {}
    Caster:SetContextThink(DoUniqueString("cirno03_in_spelling"), function()
        if GameRules:IsGamePaused() then
            return 0.03
        end
        local VecPos = VecStart + Direction * MovePerTick * tick
        local enemies = FindUnitsInRadius(Caster:GetTeamNumber(), VecPos, nil, keys.width,
...
function OnCirno04SpellStart(keys)
    ...
    local AOE_damage_table = {
        victim = nil,
        attacker = Caster,
        damage = Ability:GetSpecialValueFor("damage") * TickInterval,
        damage_type = Ability:GetAbilityDamageType(),
        ability = Ability
    }

    local effectIndex = ParticleManager:CreateParticle("particles/heroes/cirno/ability_cirno_04.vpcf",
        PATTACH_CUSTOMORIGIN, nil)
    ...
    Caster:SetContextThink("cirno04_in_spelling", function()
        ...
        local enemies = FindUnitsInRadius(Caster:GetTeamNumber(), Caster:GetOrigin(), nil, keys.radius,
            Ability:GetAbilityTargetTeam(), Ability:GetAbilityTargetType(), Ability:GetAbilityTargetFlags(),
            FIND_ANY_ORDER, --[[FIND_CLOSEST,]] false)
        for _, v in pairs(enemies) do
            ...
            if bApplyModifier and v and not v:IsNull() then
                if not v:HasModifier(keys.debuff_freeze_name) then
                    ...
                    if new_stack >= keys.stack_slow_max then
                        Ability:ApplyDataDrivenModifier(Caster, v, keys.debuff_freeze_name, {
                            duration = left_time
                        })
                        ...
                        local effectIndex = ParticleManager:CreateParticle(
                            "particles/heroes/cirno/ability_cirno_04_buff.vpcf", PATTACH_CUSTOMORIGIN, v)
...
function Cirnowanbaochui(keys)
    ...
    if (caster:IsRealHero() and target:IsHero() == true and caster:HasModifier("modifier_item_wanbaochui") and
        casterName == "npc_dota_hero_axe") then
        local modifier = target:FindModifierByName("Cirno_wanbaochui_debuff")
        if modifier == nil then
            ability:ApplyDataDrivenModifier(caster, target, "Cirno_wanbaochui_debuff", {
                duration = ability:GetSpecialValueFor("intellect_duration")
            })
            ...
            modifier:IncrementStackCount()
            modifier:SetDuration(ability:GetSpecialValueFor("intellect_duration"), true)
            ability:ApplyDataDrivenModifier(caster, target, "Cirno_wanbaochui_debuff_ex", {
                duration = 0.03
            })
        end

        if caster:GetIntellect(false) < target:GetIntellect(false) then
            local dealdamage = target:GetIntellect(false) - caster:GetIntellect(false)
            local damage_table = {
            ...
            UtilStun:UnitStunTarget(caster, target, ability:GetSpecialValueFor("intellect_stun"))
