	...
	if keys.attacker == self:GetCaster() and keys.attacker:GetTeamNumber() ~= keys.target:GetTeamNumber() and not keys.target:IsBuilding() then
		...
		if caster:HasModifier("modifiery_thdots_shinki_02_buff") and caster:HasModifier("modifier_item_wanbaochui") and caster:IsRealHero() then
			local damage_table=
			{
				victim=target,
				attacker=caster,
				damage          = ability:GetSpecialValueFor("damage") + (caster:GetIntellect(false)*int_bonus) +(caster:GetMaxHealth() * (ability:GetSpecialValueFor("percentage")/100)) ,
	...
	for _,unit in pairs(enemy) do
		if  caster:HasModifier("modifiery_thdots_shinki_02_buff")		then
			...
			local damage_table=
			{
				victim=unit,
				attacker=caster,
				damage          = self:GetSpecialValueFor("damage") + (caster:GetIntellect(false)*int_bonus) +(caster:GetMaxHealth() * (self:GetSpecialValueFor("percentage")/100)) ,
			...
			local damage_table=
			{
				victim=unit,
				attacker=caster,
				damage          = self:GetSpecialValueFor("damage") + (caster:GetIntellect(false)*int_bonus),
				damage_type     = self:GetAbilityDamageType(),
				damage_flags    = self:GetAbilityTargetFlags(),
				ability= ability
			}
...
function modifiery_thdots_shinki_02_buff:GetModifierAttackRangeBonus ()
	if self:GetParent():IsRangedAttacker() then
		local ability = self:GetCaster():FindAbilityByName("special_bonus_unique_shinki_1")
		if ability then
...
function ability_thdots_shinki_ultimate:OnSpellStart()
	...
	self.caster:SetContextThink("shinki4",
		function()
			self.illusion = CreateIllusionTHD(self,self.target,nil,incoming_damage,outgoing_damage,duration,true)
			...
			self.illusion:SetMana(self.illusion:GetMaxMana())
