...
function CustomChat:UpdateHeroRanksResponse(event)
	...
	ProtectedCustomEvents:SendToPlayer(player, "custom_chat:update_hero_ranks", self.hero_ranks)
end
...
function CustomChat:UpdateHeroRanks(event)
	...
	ProtectedCustomEvents:SendToAllClients("custom_chat:update_hero_ranks", self.hero_ranks)
end
...
function CustomChat:UpdateClient(event)
	...
	ProtectedCustomEvents:SendToPlayer(player, "custom_chat:update_client", {
...
function CustomChat:MessageToPlayer(sender_id, target_id, main_token, tokens, abilities, extra_data)
	...
	ProtectedCustomEvents:SendToPlayer(target, "custom_chat:message", {
		sender_id = sender_id,
...
function CustomChat:MessageToAll(sender_id, main_token, tokens, abilities, extra_data)
	...
	ProtectedCustomEvents:SendToAllClients("custom_chat:message", {
		sender_id = sender_id,
		main_token = main_token,
		tokens = tokens,
		abilities = abilities,
...
function CustomChat:MessageToTeam(sender_id, team_number, main_token, tokens, abilities, extra_data)
	...
	ProtectedCustomEvents:SendToTeam(team_number, "custom_chat:message", {
		sender_id = sender_id,
		main_token = main_token,
		tokens = tokens,
		abilities = abilities,
