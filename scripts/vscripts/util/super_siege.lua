...
function modifier_ability_thdots_super_siege:OnCreated()
	...
	if self:GetParent():GetUnitName() == "npc_thd_goodguys_super_siege" then
		local particle = ParticleManager:CreateParticle("particles/econ/courier/courier_trail_orbit/courier_trail_orbit.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent())
		ParticleManager:ReleaseParticleIndex(particle)
	elseif self:GetParent():GetUnitName() == "npc_thd_badguys_super_siege" then
		local particle = ParticleManager:CreateParticle("particles/units/heroes/hero_spectre/spectre_desolate_debuff.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent())
		ParticleManager:ReleaseParticleIndex(particle)
	end
end
	...
	GameRules:GetGameModeEntity():SetContextThink("siege_time", 
		function()
			...
			if math.floor(GameRules:GetDOTATime(false, false)) >= START_TIME then
				...
				if say then
					GameRules:SendCustomMessage("#SuperSiege_Explain1",0,0)
					GameRules:SendCustomMessage("#SuperSiege_Explain2",0,0)
					say = false
				end
				CreateSiege_Hakurei()
				CreateSiege_Moriya()
